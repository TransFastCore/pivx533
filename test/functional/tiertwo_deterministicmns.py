...
"""Test deterministic patriotnodes"""
...
class DIP3Test(TrumpCoinTestFramework):
    ...
    def check_mn_list(self, mns):
        ...
        self.log.info("Deterministic list contains %d patriotnodes for all peers." % len(mns))

    def check_mn_enabled_count(self, enabled, total):
        for node in self.nodes:
            node_count = node.getpatriotnodecount()
    ...
    def get_last_paid_mn(self):
        return next(x['proTxHash'] for x in self.nodes[0].listpatriotnodes()
    ...
    def wait_until_mnsync_completed(self):
        ...
        while synced != SYNC_FINISHED and time.time() < timeout:
            synced = [node.mnsync("status")["RequestedPatriotnodeAssets"]
                      for node in self.nodes]
    ...
    def run_test(self):
        ...
        assert_equal("success", self.set_spork(self.minerPos, "SPORK_21_LEGACY_PNS_MAX_HEIGHT", 130))
        time.sleep(1)
        assert_equal([130] * self.num_nodes, [self.get_spork(x, "SPORK_21_LEGACY_PNS_MAX_HEIGHT")
        ...
        # enabled/total patriotnodes: 0/0
        self.check_mn_enabled_count(0, 0)

        # Create 3 DPNs and init the remote nodes
        self.log.info("Initializing patriotnodes...")
        self.add_new_dmn(mns, "internal")
        self.add_new_dmn(mns, "external")
        self.add_new_dmn(mns, "fund")
        for mn in mns:
            self.nodes[mn.idx].initpatriotnode(mn.operator_sk, "", True)
            time.sleep(1)
        miner.generate(1)
        self.sync_blocks()

        # enabled/total patriotnodes: 3/3
        self.check_mn_enabled_count(3, 3)

        # Init the other 3 remote nodes before creating the ProReg tx
        self.log.info("Initializing more patriotnodes...")
        op_keys = []
        for i in range(3):
            idx = 2 + len(mns) + i
            bls_keypair = controller.generateblskeypair()
            self.nodes[idx].initpatriotnode(bls_keypair["secret"], "", True)
        ...
        self.sync_blocks()
        time.sleep(1)
        self.log.info("Patriotnodes started.")

        # enabled/total patriotnodes: 6/6
        self.check_mn_enabled_count(6, 6)
        self.check_mn_list(mns)

        # Check status from remote nodes
        assert_equal([self.nodes[idx].getpatriotnodestatus()['status'] for idx in range(2, self.num_nodes)],
                     ["Ready"] * (self.num_nodes - 2))
        self.log.info("All patriotnodes ready.")
        ...
        # enabled/total patriotnodes: 5/5
        ...
        # enabled/total patriotnodes: 6/6
        self.check_mn_enabled_count(6, 6)
        self.check_mn_list(mns)     # 6 patriotnodes again

        # Test payments.
        # Mine 12 blocks and check that each patriotnode has been paid exactly twice.
        # Save last paid patriotnode. Check that it's the last paid also after the 12 blocks.
        # Note: dmn2 sends (2 * 0.3 TRUMP) to the operator, and (2 * 2.7 TRUMP) to the owner
        self.log.info("Testing patriotnode payments...")
        ...
        self.log.info("All patriotnodes paid twice.")
        ...
        self.log.info("Testing invalid patriotnode payment...")
        ...
        # Test ProUpServ txes
        self.log.info("Trying to update a non-existent patriotnode...")
        ...
        assert_raises_rpc_error(-5, "invalid TrumpCoin address InvalidPayee",
        ...
        miner.generate(1)
        self.sync_blocks()
        # Then send the ProUpServ tx from the patriotnode
        ...
        assert_raises_rpc_error(-5, "invalid TrumpCoin address InvalidPayee", controller.protx_update_registrar,
        ...
        # enabled/total patriotnodes: 5/6
        ...
        # Test ProUpRev txes
        self.log.info("Trying to revoke a non-existent patriotnode...")
        ...
        self.log.info("Revoke patriotnode...")
        # Do it from the remote node (so no need to pass the operator BLS secret key)
        ...
        miner.generate(1)
        self.sync_blocks()
        # Then send the ProUpRev tx from the patriotnode
        ...
        self.log.info("Revoke patriotnode (with external key)...")
        ...
        # enabled/total patriotnodes: 3/6 (mn0 banned, mn3 and mn4 revoked)
        ...
        # Test reviving a patriotnode
        self.log.info("Reviving a patriotnode...")
        bls_keypair = controller.generateblskeypair()
        ...
        # enabled/total patriotnodes: 4/6 (mn3 is back)
