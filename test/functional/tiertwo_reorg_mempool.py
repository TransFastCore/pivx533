...
"""
Test deterministic patriotnodes conflicts and reorgs.
...
class TiertwoReorgMempoolTest(TrumpCoinTestFramework):
    ...
    def register_patriotnode(self, from_node, dmn, collateral_addr):
    ...
    def run_test(self):
        ...
        nodeA = self.nodes[0]
        nodeB = self.nodes[1]
        free_idx = 1  # unique id for patriotnodes. first available.

        # Enforce mn payments and reject legacy mns at block 202
        self.activate_spork(0, "SPORK_8_PATRIOTNODE_PAYMENT_ENFORCEMENT")
        assert_equal("success", self.set_spork(0, "SPORK_21_LEGACY_PNS_MAX_HEIGHT", 201))
        time.sleep(1)
        assert_equal([201] * self.num_nodes, [self.get_spork(x, "SPORK_21_LEGACY_PNS_MAX_HEIGHT")
        ...
        # Register two patriotnodes before the split
        ...
        self.register_patriotnode(nodeA, pre_split_mn1, collateral_addr)
        self.register_patriotnode(nodeA, pre_split_mn2, collateral_addr)
        nodeA.generate(1)
        self.sync_blocks()
        ...
        self.log.info("Pre-split patriotnodes registered.")
        ...
        # Register 5 patriotnodes, then mine 5 blocks
        self.log.info("Registering patriotnodes on chain A...")
        for _ in range(5):
            dmn = create_new_dmn(free_idx, nodeA, collateral_addr, None)
            free_idx += 1
            self.register_patriotnode(nodeA, dmn, collateral_addr)
            mnsA.append(dmn)
        nodeA.generate(5)
        self.check_mn_list_on_node(0, mnsA)
        self.log.info("Patriotnodes registered on chain A.")

        # Now create a collateral (which will be used later to register a patriotnode)
        ...
        free_idx += 1
        self.register_patriotnode(nodeA, mempool_dmn1, collateral_addr)
        ...
        assert_raises_rpc_error(-26, "protx-dup",
                                self.register_patriotnode, nodeA, dmn_A1, collateral_addr)
        ...
        assert_raises_rpc_error(-26, "protx-dup",
                                self.register_patriotnode, nodeA, dmn_A2, collateral_addr)
        ...
        assert_raises_rpc_error(-26, "protx-dup",
                                self.register_patriotnode, nodeA, dmn_A3, collateral_addr)
        ...
        free_idx += 1
        self.register_patriotnode(nodeA, mempool_dmn2, collateral_addr)
        self.register_patriotnode(nodeA, mempool_dmn3, collateral_addr)
        ...
        self.log.info("Revoking two patriotnodes...")
        ...
        self.log.info("Registering patriotnodes on chain B...")
        ...
        for dmn in [dmn_A1, dmn_A2, dmn_A3]:
            self.register_patriotnode(nodeB, dmn, collateral_addr)
            mnsB.append(dmn)
            nodeB.generate(1)
        self.check_mn_list_on_node(1, mnsB)

        # Pick the proReg for the first PN registered on chain A, and replay it on chain B
        self.log.info("Replaying a patriotnode on a different chain...")
        ...
        # Now pick a proReg for another PN registered on chain A, and re-register it on chain B
        self.log.info("Re-registering a patriotnode on a different chain...")
        rereg_mn = random.choice(mnsA)
        mnsA.remove(rereg_mn)
        self.register_patriotnode(nodeB, rereg_mn, collateral_addr)
        mnsB.append(rereg_mn)   # changed proTx hash
        nodeB.generate(1)
        self.check_mn_list_on_node(1, mnsB)

        # Register 5 more patriotnodes. One per block.
        for _ in range(5):
        ...
        # Register one patriotnode reusing the IP of the proUpServ mempool tx on chainA
        dmn1000 = create_new_dmn(free_idx, nodeB, collateral_addr, None)
        free_idx += 1
        dmn1000.ipport = "127.0.0.1:1000"
        mnsB.append(dmn1000)
        self.register_patriotnode(nodeB, dmn1000, collateral_addr)

        # Register one patriotnode reusing the operator-key of the proUpReg mempool tx on chainA
        dmnop = create_new_dmn(free_idx, nodeB, collateral_addr, None)
        free_idx += 1
        dmnop.operator_pk = operator_to_reuse
        mnsB.append(dmnop)
        self.register_patriotnode(nodeB, dmnop, collateral_addr)

        # Then mine 10 more blocks on chain B
        nodeB.generate(10)
        self.check_mn_list_on_node(1, mnsB)
        self.log.info("Patriotnodes registered on chain B.")
        ...
        self.log.info("Checking patriotnode list...")
        ...
        # conflicts with the patriotnodes of chain B, now connected.
        ...
        # now used by a newly connected patriotnode.
        # The second mempool proUpServ tx has been removed as it was meant to update
        # a patriotnode that is not in the deterministic list anymore.
        ...
        # proupserv3 has changed the IP of the pre_split patriotnode 1
        ...
        # prouprev2 has revoked pre_split patriotnode 2
        ...
        self.log.info("Both nodes have %d registered patriotnodes." % len(mns_all))
