...
TMPDIR_PREFIX = "trumpcoin_func_test_"


class TrumpCoinTestFramework():
    """Base class for a trumpcoin test script.

    Individual trumpcoin test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def main(self):
        ...
        parser.add_option("--nocleanup", dest="nocleanup", default=False, action="store_true",
                          help="Leave trumpcoinds and test.* datadir on exit or error")
        parser.add_option("--noshutdown", dest="noshutdown", default=False, action="store_true",
                          help="Don't stop trumpcoinds after the test execution")
        parser.add_option("--srcdir", dest="srcdir", default=os.path.normpath(os.path.dirname(os.path.realpath(__file__))+"/../../../src"),
                          help="Source directory containing trumpcoind/trumpcoin-cli (default: %default)")
        ...
        parser.add_option("--usecli", dest="usecli", default=False, action="store_true",
                          help="use trumpcoin-cli instead of RPC for all commands")
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: trumpcoinds were not stopped and may still be running")
    ...
    def stop_node(self, i):
        """Stop a trumpcoind test node"""
        self.nodes[i].stop_node()
        self.nodes[i].wait_until_stopped()

    def stop_nodes(self):
        """Stop multiple trumpcoind test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as trumpcoind's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        def copy_cachedir(origin, destination, num_nodes=MAX_NODES):
            for i in range(num_nodes):
                ...
                initialize_datadir(destination, i)  # Overwrite port/rpcport in trumpcoin.conf
        ...
        def clone_cache_from_node_1(cachedir, from_num=4):
            ...
            for i in range(from_num, MAX_NODES):
                ...
                initialize_datadir(cachedir, i)  # Overwrite port/rpcport in trumpcoin.conf
        ...
        def start_nodes_from_dir(ddir, num_nodes=MAX_NODES):
            ...
            for i in range(num_nodes):
                ...
                args = [os.getenv("BITCOIND", "trumpcoind"), "-spendzeroconfchange=1", "-server", "-keypool=1",
        ...
        def generate_pow_cache():
            ...
            # Create cache directories, run trumpcoinds:
    ...
    # TrumpCoin Specific TestFramework
    ...
    def get_mn_lastseen(self, node, mnTxHash):
        mnData = node.listpatriotnodes(mnTxHash)
        if len(mnData) == 0:
            return -1
        ...
        while synced != SYNC_FINISHED and time.time() < timeout:
            for i in range(self.num_nodes):
                if synced[i] != SYNC_FINISHED[i]:
                    synced[i] = self.nodes[i].mnsync("status")["RequestedPatriotnodeAssets"]
    ...
    def controller_start_patriotnode(self, mnOwner, patriotnodeAlias):
        ret = mnOwner.startpatriotnode("alias", "false", patriotnodeAlias, True)
    ...
    def setupDPN(self,
        ...
        self.log.info("Creating%s proRegTx for deterministic patriotnode..." % (
    ...
    def setupPatriotnode(self,
                        ...
                        mnRemotePos,
                        patriotnodePrivKey):
        self.log.info("adding balance to the mn owner for " + patriotnodeAlias + "..")
        mnAddress = mnOwner.getnewaddress(patriotnodeAlias)
        ...
        assert_greater_than(collateralTxId_n, -1)
        assert_greater_than(json_tx["confirmations"], 0)
        # update patriotnode file
        self.log.info("collateral accepted for " + patriotnodeAlias + ". Updating patriotnode.conf...")
        confData = "%s 127.0.0.1:%d %s %s %d" % (patriotnodeAlias,
                                                 p2p_port(mnRemotePos),
                                                 patriotnodePrivKey,
                                                 collateralTxId,
                                                 collateralTxId_n)
        destPath = os.path.join(mnOwnerDirPath, "patriotnode.conf")
    ...
    """
    Create a ProReg tx, which references an 100 TRUMP UTXO as collateral.
    ...
    """ Create and register new deterministic patriotnode
    ...
    :return: dmn:              (Patriotnode) the deterministic patriotnode object
    """
    def register_new_dmn(self, idx, miner_idx, controller_idx, strType,
        ...
        # Create ProRegTx
        self.log.info("Creating%s proRegTx for deterministic patriotnode idx=%d..." % (
    ...
    def check_mn_list_on_node(self, idx, mns):
        ...
        mnlist = self.nodes[idx].listpatriotnodes()
...
'''
TrumpCoinTestFramework extensions
'''
# !TODO: remove after obsoleting legacy system
class TrumpCoinTier2TestFramework(TrumpCoinTestFramework):
    ...
    def set_test_params(self):
        ...
        # Updated in setup_3_patriotnodes_network() to be called at the start of run_test
        ...
        self.remoteDPN1 = None       # self.nodes[self.remoteDPN1Pos]
    ...
    def controller_start_all_patriotnodes(self):
        ...
        self.log.info("patriotnodes enabled and running properly!")
    ...
    def setup_3_patriotnodes_network(self):
        ...
        self.remoteDPN1 = self.nodes[self.remoteDPN1Pos]
        ...
        self.log.info("patriotnodes setup..")
        # setup first patriotnode node, corresponding to nodeOne
        self.mnOneCollateral = self.setupPatriotnode(
            self.ownerOne,
            self.miner,
            self.patriotnodeOneAlias,
            os.path.join(ownerOneDir, "regtest"),
            self.remoteOnePos,
            self.mnOnePrivkey)
        # setup second patriotnode node, corresponding to nodeTwo
        self.mnTwoCollateral = self.setupPatriotnode(
            self.ownerTwo,
            self.miner,
            self.patriotnodeTwoAlias,
            os.path.join(ownerTwoDir, "regtest"),
            self.remoteTwoPos,
            self.mnTwoPrivkey)
        # setup deterministic patriotnode
        self.proRegTx1, self.dmn1Privkey = self.setupDPN(
            self.ownerOne,
            self.miner,
            self.remoteDPN1Pos,
            "fund"
        ...
        self.log.info("patriotnodes setup completed, initializing them..")

        # now both are configured, let's activate the patriotnodes
        self.stake(1)
        time.sleep(3)
        self.advance_mocktime(10)
        remoteOnePort = p2p_port(self.remoteOnePos)
        remoteTwoPort = p2p_port(self.remoteTwoPos)
        self.remoteOne.initpatriotnode(self.mnOnePrivkey, "127.0.0.1:"+str(remoteOnePort))
        self.remoteTwo.initpatriotnode(self.mnTwoPrivkey, "127.0.0.1:"+str(remoteTwoPort))
        self.remoteDPN1.initpatriotnode(self.dmn1Privkey, "", True)

        # wait until mnsync complete on all nodes
        self.stake(1)
        self.wait_until_mnsync_finished()
        self.log.info("tier two synced! starting patriotnodes..")

        # Now everything is set, can start both patriotnodes
        self.controller_start_all_patriotnodes()
