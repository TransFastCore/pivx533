...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/TrumpCoinParams"
    else
        PARAMS_DIR="$HOME/.trumpcoin-params"
    fi
fi
...
Failed to fetch the TrumpCoin zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
TrumpCoin - fetch-params.sh

This script will fetch the TrumpCoin zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common TrumpCoin zkSNARK parameters. Note that it is
