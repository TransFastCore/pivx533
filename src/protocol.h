...
// Copyright (c) 2016-2020 The TrumpCoin developers
...
/**
 * The mnbroadcast message is used to broadcast patriotnode startup data to connected peers
 */
extern const char* PNBROADCAST;
/**
 * The mnbroadcast2 message is used to broadcast patriotnode startup data to connected peers
 * Supporting BIP155 node addresses.
 */
extern const char* PNBROADCAST2;
/**
 * The mnping message is used to ensure a patriotnode is still active
 */
extern const char* PNPING;
/**
 * The mnwinner message is used to relay and distribute consensus for patriotnode
 * payout ordering
 */
extern const char* PNWINNER;
/**
 * The getmnwinners message is used to request winning patriotnode data from connected peers
 */
extern const char* GETPNWINNERS;
/**
* The dseg message is used to request the Patriotnode list or an specific entry
*/
extern const char* GETPNLIST;
/**
...
enum ServiceFlags : uint64_t {
    ...
    // NODE_BLOOM_WITHOUT_PN means the node has the same features as NODE_BLOOM with the only difference
    // that the node doesn't want to receive PatriotNodes messages. (the 1<<3 was not picked as constant because on bitcoin 0.14 is witness and we want that update here )
    NODE_BLOOM_WITHOUT_PN = (1 << 4),
...
enum GetDataMsg
{
    ...
    MSG_SPORK,
    MSG_PATRIOTNODE_WINNER,
    MSG_PATRIOTNODE_SCANNING_ERROR,
    MSG_BUDGET_VOTE,
    MSG_BUDGET_PROPOSAL,
    MSG_BUDGET_FINALIZED,
    MSG_BUDGET_FINALIZED_VOTE,
    MSG_PATRIOTNODE_QUORUM,
    MSG_PATRIOTNODE_ANNOUNCE,
    MSG_PATRIOTNODE_PING,
    MSG_DSTX,
