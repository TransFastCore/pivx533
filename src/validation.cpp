...
#if defined(NDEBUG)
#error "TrumpCoin cannot be compiled without assertions."
#endif
...
{
    util::ThreadRename("trumpcoin-scriptch");
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false)
{
    ...
    if (!fInitialBlockDownload) {
        if (!IsBlockPayeeValid(block, pindex->pprev)) {
            ...
            return state.DoS(0, false, REJECT_INVALID, "bad-cb-payee", false, "Couldn't find patriotnode/budget payment");
        }
    }
...
bool CheckColdStakeFreeOutput(const CTransaction& tx, const int nHeight)
{
    ...
    // If the last output is different, then it can be either a patriotnode
    ...
    if (outs >=3 && lastOut.scriptPubKey != tx.vout[outs-2].scriptPubKey) {
        if (Params().GetConsensus().NetworkUpgradeActive(nHeight, Consensus::UPGRADE_V6_0)) {
            // after v6.0, patriotnode and budgets are paid in the coinbase. No more free outputs allowed.
            return false;
        }
        if (lastOut.nValue == GetPatriotnodePayment())
            return true;
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    if (pindexPrev != nullptr && block.hashPrevBlock != UINT256_ZERO) {
        ...
        if (nHeight != 0 && !IsInitialBlockDownload()) {
            ...
            LogPrintf("%s: Patriotnode/Budget payment checks skipped on sync\n", __func__);
        }
    }
...
bool CheckWork(const CBlock& block, const CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // TrumpCoin Specific reference to the block with the wrong threshold was used.
        const Consensus::Params& consensus = Params().GetConsensus();
        if ((block.nTime == (uint32_t) consensus.nTrumpCoinBadBlockTime) &&
                (block.nBits == (uint32_t) consensus.nTrumpCoinBadBlockBits)) {
            // accept TrumpCoin block minted with incorrect proof of work threshold
            return true;
        }
        ...
        for (const CTxIn& in: tx->vin) {
            ...
            if (isPrivZerocoinSpend || isPublicSpend) {
                ...
                if (isPublicSpend) {
                    ...
                    if (!ZTRUMPModule::ParseZerocoinPublicSpend(in, *tx, state, publicSpend)){
                        return false;
                    }
                    spend = publicSpend;
                } else {
