...
// Copyright (c) 2017-2020 The TrumpCoin developers
    ...
    try
    {
        while (pc < pend)
        {
            ...
            switch (opcode)
            {
                ...
                case OP_EQUAL:
                case OP_EQUALVERIFY:
                //case OP_NOTEQUAL: // use OP_NUPNOTEQUAL
                {
                    // (x1 x2 - bool)
                    if (stack.size() < 2)
                        return set_error(serror, SCRIPT_ERR_INVALID_STACK_OPERATION);
                ...
                case OP_MAX:
                {
                    ...
                    case OP_NUPNOTEQUAL:         bn = (bn1 != bn2); break;
...
const unsigned char TrumpCoin_PREVOUTS_HASH_PERSONALIZATION[crypto_generichash_blake2b_PERSONALBYTES] =
        {'P','I','V','X','P','r','e','v','o','u','t','H','a','s','h'};
const unsigned char TrumpCoin_SEQUENCE_HASH_PERSONALIZATION[crypto_generichash_blake2b_PERSONALBYTES] =
        {'P','I','V','X','S','e','q','u','e','n','c','H','a','s','h'};
const unsigned char TrumpCoin_OUTPUTS_HASH_PERSONALIZATION[crypto_generichash_blake2b_PERSONALBYTES] =
        {'P','I','V','X','O','u','t','p','u','t','s','H','a','s','h'};
const unsigned char TrumpCoin_SHIELDED_SPENDS_HASH_PERSONALIZATION[crypto_generichash_blake2b_PERSONALBYTES] =
        {'P','I','V','X','S','S','p','e','n','d','s','H','a','s','h'};
const unsigned char TrumpCoin_SHIELDED_OUTPUTS_HASH_PERSONALIZATION[crypto_generichash_blake2b_PERSONALBYTES] =
        {'P','I','V','X','S','O','u','t','p','u','t','H','a','s','h'};



uint256 GetPrevoutHash(const CTransaction& txTo) {
    CBLAKE2bWriter ss(SER_GETHASH, 0, TrumpCoin_PREVOUTS_HASH_PERSONALIZATION);
...
uint256 GetSequenceHash(const CTransaction& txTo) {
    CBLAKE2bWriter ss(SER_GETHASH, 0, TrumpCoin_SEQUENCE_HASH_PERSONALIZATION);
    for (unsigned int n = 0; n < txTo.vin.size(); n++) {
...
uint256 GetOutputsHash(const CTransaction& txTo) {
    CBLAKE2bWriter ss(SER_GETHASH, 0, TrumpCoin_OUTPUTS_HASH_PERSONALIZATION);
...
uint256 GetShieldedSpendsHash(const CTransaction& txTo) {
    assert(txTo.sapData);
    CBLAKE2bWriter ss(SER_GETHASH, 0, TrumpCoin_SHIELDED_SPENDS_HASH_PERSONALIZATION);
...
uint256 GetShieldedOutputsHash(const CTransaction& txTo) {
    assert(txTo.sapData);
    CBLAKE2bWriter ss(SER_GETHASH, 0, TrumpCoin_SHIELDED_OUTPUTS_HASH_PERSONALIZATION);
    auto sapData = txTo.sapData;
...
uint256 SignatureHash(const CScript& scriptCode, const CTransaction& txTo, unsigned int nIn, int nHashType, const CAmount& amount, SigVersion sigversion, const PrecomputedTransactionData* cache)
{
    ...
    if (sigversion == SIGVERSION_SAPLING) {
        ...
        memcpy(personalization, "TrumpCoinSigHash", 12);
...
bool TransactionSignatureChecker::CheckColdStake(bool fAllowLastOutputFree, const CScript& prevoutScript, std::vector<valtype>& stack, unsigned int flags, ScriptError* serror) const
{
    ...
    // budget/patriotnode payments (before v6.0 enforcement), and is checked in CheckColdstakeFreeOutput().
