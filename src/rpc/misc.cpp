...
UniValue getinfo(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,           (numeric) the total trumpcoin balance of the wallet\n"
...
UniValue mnsync(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1 || (strMode != "status" && strMode != "reset")) {
        throw std::runtime_error(
            ...
            "  \"lastPatriotnodeList\": xxxx,        (numeric) Timestamp of last PN list message\n"
            "  \"lastPatriotnodeWinner\": xxxx,      (numeric) Timestamp of last PN winner message\n"
            "  \"lastBudgetItem\": xxxx,            (numeric) Timestamp of last PN budget message\n"
            "  \"lastFailure\": xxxx,           (numeric) Timestamp of last failed sync\n"
            "  \"nCountFailures\": n,           (numeric) Number of failed syncs (total)\n"
            "  \"sumPatriotnodeList\": n,        (numeric) Number of PN list messages (total)\n"
            "  \"sumPatriotnodeWinner\": n,      (numeric) Number of PN winner messages (total)\n"
            "  \"sumBudgetItemProp\": n,        (numeric) Number of PN budget messages (total)\n"
            "  \"sumBudgetItemFin\": n,         (numeric) Number of PN budget finalization messages (total)\n"
            "  \"countPatriotnodeList\": n,      (numeric) Number of PN list messages (local)\n"
            "  \"countPatriotnodeWinner\": n,    (numeric) Number of PN winner messages (local)\n"
            "  \"countBudgetItemProp\": n,      (numeric) Number of PN budget messages (local)\n"
            "  \"countBudgetItemFin\": n,       (numeric) Number of PN budget finalization messages (local)\n"
            "  \"RequestedPatriotnodeAssets\": n, (numeric) Status code of last sync phase\n"
            "  \"RequestedPatriotnodeAttempt\": n, (numeric) Status code of last sync attempt\n"
            "}\n"
    ...
    if (strMode == "status") {
        ...
        obj.pushKV("IsBlockchainSynced", patriotnodeSync.IsBlockchainSynced());
        obj.pushKV("lastPatriotnodeList", patriotnodeSync.lastPatriotnodeList);
        obj.pushKV("lastPatriotnodeWinner", patriotnodeSync.lastPatriotnodeWinner);
        obj.pushKV("lastBudgetItem", patriotnodeSync.lastBudgetItem);
        obj.pushKV("lastFailure", patriotnodeSync.lastFailure);
        obj.pushKV("nCountFailures", patriotnodeSync.nCountFailures);
        obj.pushKV("sumPatriotnodeList", patriotnodeSync.sumPatriotnodeList);
        obj.pushKV("sumPatriotnodeWinner", patriotnodeSync.sumPatriotnodeWinner);
        obj.pushKV("sumBudgetItemProp", patriotnodeSync.sumBudgetItemProp);
        obj.pushKV("sumBudgetItemFin", patriotnodeSync.sumBudgetItemFin);
        obj.pushKV("countPatriotnodeList", patriotnodeSync.countPatriotnodeList);
        obj.pushKV("countPatriotnodeWinner", patriotnodeSync.countPatriotnodeWinner);
        obj.pushKV("countBudgetItemProp", patriotnodeSync.countBudgetItemProp);
        obj.pushKV("countBudgetItemFin", patriotnodeSync.countBudgetItemFin);
        obj.pushKV("RequestedPatriotnodeAssets", patriotnodeSync.RequestedPatriotnodeAssets);
        obj.pushKV("RequestedPatriotnodeAttempt", patriotnodeSync.RequestedPatriotnodeAttempt);
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            "validateaddress \"trumpcoinaddress\"\n"
            "\nReturn information about the given trumpcoin address.\n"

            "\nArguments:\n"
            "1. \"trumpcoinaddress\"     (string, required) The trumpcoin address to validate\n"

            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"trumpcoinaddress\",    (string) The trumpcoin address validated\n"
            "  \"scriptPubKey\" : \"hex\",       (string) The hex encoded scriptPubKey generated by the address -only if is standard address-\n"
            "  \"ismine\" : true|false,          (boolean) If the address is yours or not\n"
            "  \"isstaking\" : true|false,       (boolean) If the address is a staking address for TrumpCoin cold staking -only if is standard address-\n"
...
#ifdef ENABLE_WALLET
        // Case 1: TrumpCoin address and we have full public key:
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "2. \"keys\"       (string, required) A json array of keys which are trumpcoin addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) trumpcoin address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            "verifymessage \"trumpcoinaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"

            "\nArguments:\n"
            "1. \"trumpcoinaddress\"  (string, required) The trumpcoin address to use for the signature.\n"
...
UniValue echo(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "trumpcoin-cli and the GUI. There is no server-side difference."
        );
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ --------
    ...
    { "util",               "validateaddress",        &validateaddress,        true,  {"trumpcoinaddress"} }, /* uses wallet if enabled */
    { "util",               "verifymessage",          &verifymessage,          true,  {"trumpcoinaddress","signature","message"} },
