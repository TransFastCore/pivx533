...
UniValue preparebudget(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 6)
        throw std::runtime_error(
            ...
            "5. \"address\":     (string, required) TrumpCoin address to send payments to\n"
            ...
            "\nExamples:\n" +
            HelpExampleCli("preparebudget", "\"test-proposal\" \"https://forum.trumpcoin.org/t/test-proposal\" 2 820800 \"D9oc6C3dttUbv8zd7zGNq1qKBGf4ZQ1XEE\" 500") +
            HelpExampleRpc("preparebudget", "\"test-proposal\" \"https://forum.trumpcoin.org/t/test-proposal\" 2 820800 \"D9oc6C3dttUbv8zd7zGNq1qKBGf4ZQ1XEE\" 500"));
    ...
    if (!pwallet->CreateBudgetFeeTX(wtx, nHash, keyChange, false)) { // 50 TRUMP collateral for proposal
...
UniValue submitbudget(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 7)
        throw std::runtime_error(
            ...
            "\nExamples:\n" +
            HelpExampleCli("submitbudget", "\"test-proposal\" \"https://forum.trumpcoin.org/t/test-proposal\" 2 820800 \"D9oc6C3dttUbv8zd7zGNq1qKBGf4ZQ1XEE\" 500") +
            HelpExampleRpc("submitbudget", "\"test-proposal\" \"https://forum.trumpcoin.org/t/test-proposal\" 2 820800 \"D9oc6C3dttUbv8zd7zGNq1qKBGf4ZQ1XEE\" 500"));
    ...
    if (!patriotnodeSync.IsBlockchainSynced()) {
        throw std::runtime_error("Must wait for client to sync with patriotnode network. Try again in a minute or so.");
    }
...
UniValue mnbudgetvote(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || (request.params.size() == 3 && (strCommand != "local" && strCommand != "many")) || (request.params.size() == 4 && strCommand != "alias") ||
        ...
        throw std::runtime_error(
            ...
            "\nAfter V6 enforcement, the deterministic patriotnode system is used by default. Set the \"legacy\" parameter to true to vote with legacy patriotnodes."

            "\nArguments:\n"
            "1. \"mode\"      (string, required) The voting mode. 'local' for voting directly from a patriotnode, 'many' for voting with a PN controller and casting the same vote for each PN, 'alias' for voting with a PN controller and casting a vote for a single PN\n"
            "2. \"hash\"      (string, required) The budget proposal hash\n"
            "3. \"votecast\"  (string, required) Your vote. 'yes' to vote for the proposal, 'no' to vote against\n"
            "4. \"alias\"     (string, required for 'alias' mode) The PN alias to cast a vote for (for deterministic patriotnodes it's the hash of the proTx transaction).\n"
            "5. \"legacy\"    (boolean, optional, default=false) Use the legacy patriotnode system after deterministic patriotnodes enforcement.\n"
            ...
            "  \"detail\": [\n"
            "    {\n"
            "      \"node\": \"xxxx\",      (string) 'local' or the PN alias\n"
    ...
    bool fLegacyPN = !deterministicPNManager->IsDIP3Enforced() || (request.params.size() > 4 && request.params[4].get_bool());

    if (strCommand == "local") {
        if (!fLegacyPN) {
            throw JSONRPCError(RPC_MISC_ERROR, _("\"local\" vote is no longer available with DPNs. Use \"alias\" from the wallet with the voting key."));
        }
        return mnLocalBudgetVoteInner(true, hash, false, nVote);
    }

    // DPN require wallet with voting key
    if (!fLegacyPN) {
    ...
    if (strCommand == "many" || (isAlias = strCommand == "alias")) {
        ...
        return mnBudgetVoteInner(pwallet, fLegacyPN, hash, false, nVote, mnAlias);
    }
...
UniValue getbudgetvotes(const JSONRPCRequest& request)
{
    if (request.params.size() != 1)
        throw std::runtime_error(
            ...
            "[\n"
            "  {\n"
            "    \"mnId\": \"xxxx-x\",      (string) Patriotnode's outpoint collateral transaction (hash-n)\n"
...
UniValue getbudgetprojection(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "Net Votes needs to be above Patriotnode Count divided by 10\n"
            ...
            "    \"PaymentAddress\": \"xxxx\",     (string) TrumpCoin address of payment\n"
            "    \"Ratio\": x.xxx,               (numeric) Ratio of yeas vs nays\n"
            "    \"Yeas\": n,                    (numeric) Number of yea votes\n"
            "    \"Nays\": n,                    (numeric) Number of nay votes\n"
            "    \"Abstains\": n,                (numeric) Number of abstains\n"
            "    \"TotalPayment\": xxx.xxx,      (numeric) Total payment amount in TRUMP\n"
            "    \"MonthlyPayment\": xxx.xxx,    (numeric) Monthly payment amount in TRUMP\n"
            ...
            "    \"Allotted\": xxx.xxx,           (numeric) Amount of TRUMP allotted in current period\n"
            "    \"TotalBudgetAllotted\": xxx.xxx (numeric) Total TRUMP allotted\n"
            "  }\n"
            "  ,...\n"
            "]\n"
...
UniValue getbudgetinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            "getbudgetinfo ( \"name\" )\n"
            "\nShow current patriotnode budgets\n"
...
UniValue mnbudgetrawvote(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 6)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"collat_txid\"   (string, required) Transaction hash for the patriotnode collateral\n"
            "2. collat_vout       (numeric, required) Output index for the patriotnode collateral\n"
    ...
    if (!pmn) {
        return "Failure to find patriotnode in list : " + vin.ToString();
    }
...
UniValue mnfinalbudget(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp ||
        ...
        throw std::runtime_error(
            ...
            "  vote        - Vote on a finalized budget with local patriotnode\n"
    ...
    if (strCommand == "vote-many" || strCommand == "vote") {
        ...
        bool fLegacyPN = !deterministicPNManager->IsDIP3Enforced() || (request.params.size() > 2 && request.params[2].get_bool());

        // DPN require wallet with operator keys for vote-many
        if (!fLegacyPN && strCommand == "vote-many" && !EnsureWalletIsAvailable(pwallet, false)) {
            return NullUniValue;
        }

        return (strCommand == "vote-many" ? mnBudgetVoteInner(pwallet, fLegacyPN, hash, true, CBudgetVote::VOTE_YES, nullopt)
                                          : mnLocalBudgetVoteInner(fLegacyPN, hash, true, CBudgetVote::VOTE_YES));
    }
...
UniValue checkbudgets(const JSONRPCRequest& request)
{
    ...
    if (!patriotnodeSync.IsSynced())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Patriotnode/Budget sync not finished yet");
...
UniValue cleanbudget(const JSONRPCRequest& request)
{
    ...
    if (reset) {
        patriotnodeSync.ClearFulfilledRequest();
        patriotnodeSync.Reset();
        LogPrintf("Patriotnode sync restarted\n");
    }
    return NullUniValue;
}
