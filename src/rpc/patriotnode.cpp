...
UniValue listpatriotnodes(const JSONRPCRequest& request)
{
    ...
    if (deterministicPNManager->LegacyPNObsolete()) {
        ...
        mnList.ForEachPN(false, [&](const CDeterministicPNCPtr& dmn) {
            ...
            dmn->ToJson(obj);
            if (filterPatriotnode(obj, strFilter, !dmn->IsPoSeBanned())) {
                ret.push_back(obj);
            }
        });
        return ret;
...
UniValue getpatriotnodecount (const JSONRPCRequest& request)
{
    ...
    auto infoPNs = mnodeman.getPNsInfo();

    obj.pushKV("total", infoPNs.total);
    obj.pushKV("stable", infoPNs.stableSize);
    obj.pushKV("enabled", infoPNs.enabledSize);
    obj.pushKV("inqueue", nCount);
    obj.pushKV("ipv4", infoPNs.ipv4);
    obj.pushKV("ipv6", infoPNs.ipv6);
    obj.pushKV("onion", infoPNs.onion);
...
UniValue getpatriotnodestatus(const JSONRPCRequest& request)
{
    ...
    if (fDeterministicPN) {
        ...
        UniValue mnObj(UniValue::VOBJ);
        auto dmn = deterministicPNManager->GetListAtChainTip().GetPNByOperatorKey(amninfo->pubKeyOperator);
        if (dmn) {
            dmn->ToJson(mnObj);
        }
