...
static const std::map<ProRegParam, std::string> mapParamHelp = {
        {collateralAddress,
            "%d. \"collateralAddress\"     (string, required) The TrumpCoin address to send the collateral to.\n"
        },
        {collateralHash,
        ...
        {operatorKey,
            ...
            "                                 be performed on the active patriotnode with the corresponding operator key.\n"
        },
        {operatorPayoutAddress_register,
        ...
        {ownerAddress,
            "%d. \"ownerAddress\"          (string, required) The TrumpCoin address to use for payee updates and proposal voting.\n"
        ...
        {ownerKey,
            "%d. \"ownerKey\"              (string, optional) The owner key associated with the operator address of the patriotnode.\n"
            "                                If not specified, or set to an empty string, then the mn key must be known by your wallet, in order to sign the tx.\n"
        },
        {payoutAddress_register,
            "%d. \"payoutAddress\"          (string, required) The TrumpCoin address to use for patriotnode reward payments.\n"
        },
        {payoutAddress_update,
            "%d. \"payoutAddress\"          (string, required) The TrumpCoin address to use for patriotnode reward payments.\n"
        ...
        {revocationReason,
            "%d. reason                     (numeric, optional) The reason for patriotnode service revocation. Default: 0.\n"
...
// Allows to specify TrumpCoin address or priv key (as strings). In case of TrumpCoin address, the priv key is taken from the wallet
...
static CKeyID ParsePubKeyIDFromAddress(const std::string& strAddress)
{
    ...
    if (!keyID) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, strprintf("invalid TrumpCoin address %s", strAddress));
    }
    return *keyID;
}
...
static CBLSSecretKey GetBLSSecretKey(const std::string& hexKey)
{
    ...
    // If empty, get the active patriotnode key
    CBLSSecretKey sk; CTxIn vin;
    if (!GetActiveDPNKeys(sk, vin)) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Active patriotnode key not found. Insert DPN operator private key.");
    }
    return sk;
}
...
static UniValue ProTxRegister(const JSONRPCRequest& request, bool fSignAndSend)
{
    ...
    if (request.fHelp || request.params.size() < 7 || request.params.size() > 9) {
        throw std::runtime_error(
                ...
                (fSignAndSend ? (
                        ...
                        "\nExamples:\n"
                        + HelpExampleCli("protx_register", "\"0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef\" 0 \"168.192.1.100:15110\" \"DMJRSsuU9zfyrvxVaAEFQqK4MxZg6vgeS6\" \"DMJRSsuU9zfyrvxVaAEFQqK4MxZg6vgeS6\" \"DMJRSsuU9zfyrvxVaAEFQqK4MxZg6vgeS6\" \"DMJRSsuU9zfyrvxVaAEFQqK4MxZg6vgeS6\"")
                        ) : (
                        "{                        (json object)\n"
                        ...
                        "}\n"
                        "\nExamples:\n"
                        + HelpExampleCli("protx_register_prepare", "\"0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef\" 0 \"168.192.1.100:15110\" \"DMJRSsuU9zfyrvxVaAEFQqK4MxZg6vgeS6\" \"DMJRSsuU9zfyrvxVaAEFQqK4MxZg6vgeS6\" \"DMJRSsuU9zfyrvxVaAEFQqK4MxZg6vgeS6\" \"DMJRSsuU9zfyrvxVaAEFQqK4MxZg6vgeS6\"")
                        )
                )
        );
    }
    ...
    if (coin.out.nValue != Params().GetConsensus().nPNCollateralAmt) {
...
UniValue protx_register_fund(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 6 || request.params.size() > 8) {
        throw std::runtime_error(
                ...
                "\"txid\"                        (string) The transaction id.\n"
                "\nExamples:\n"
                + HelpExampleCli("protx_register_fund", "\"DKHHBsuU9zfxxxVaqqqQqK4MxZg6vzpf8\" \"168.192.1.100:15110\" \"DMJRSsuU9zfyrvxVaAEFQqK4MxZg6vgeS6\" \"DMJRSsuU9zfyrvxVaAEFQqK4MxZg6vgeS6\" \"DMJRSsuU9zfyrvxVaAEFQqK4MxZg6vgeS6\" \"DMJRSsuU9zfyrvxVaAEFQqK4MxZg6vgeS6\"")
        );
    }
    CheckEvoUpgradeEnforcement();
...
static void AddDPNEntryToList(UniValue& ret, CWallet* pwallet, const CDeterministicPNCPtr& dmn, bool fVerbose, bool fFromWallet)
{
...
UniValue protx_list(const JSONRPCRequest& request)
{
    ...
    CDeterministicPNList mnList = deterministicPNManager->GetListForBlock(pindex);

    // Build/filter the list
    UniValue ret(UniValue::VARR);
    mnList.ForEachPN(fValidOnly, [&](const CDeterministicPNCPtr& dmn) {
        AddDPNEntryToList(ret, pwallet, dmn, fVerbose, fFromWallet);
    });
    return ret;
}
...
UniValue protx_update_service(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 4) {
        throw std::runtime_error(
                ...
                "\"txid\"                        (string) The transaction id.\n"
                "\nExamples:\n"
                + HelpExampleCli("protx_update_service", "\"0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef\" \"168.192.1.100:15110\"")
        );
    }
    CheckEvoUpgradeEnforcement();
    ...
    auto dmn = deterministicPNManager->GetListAtChainTip().GetPN(pl.proTxHash);
    if (!dmn) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, strprintf("patriotnode with hash %s not found", pl.proTxHash.ToString()));
    }
...
UniValue protx_update_registrar(const JSONRPCRequest& request)
{
    ...
    if (!deterministicPNManager->LegacyPNObsolete()) {
        throw JSONRPCError(RPC_MISC_ERROR, "Legacy patriotnode system still active. ProUpReg transactions are not accepted yet.");
    }
...
void RegisterEvoRPCCommands(CRPCTable &tableRPC)
{
    if (!Params().IsRegTestNet()) {
        // Disabled before TrumpCoin v6.0
        return;
    }
