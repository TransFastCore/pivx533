...
private:
    ...
    // Validation
    bool CheckInputs(CPatriotnodeBroadcast& mnb, int nChainHeight, int& nDoS);
...
public:
    ...
    struct PNsInfo {
        // All the known PNs
        int total{0};
        // enabled PNs eligible for payments. Older than 8000 seconds.
        int stableSize{0};
        // PNs enabled.
        int enabledSize{0};
    ...
    // Return an overall status of the PNs list
    CPatriotnodeMan::PNsInfo getPNsInfo() const;
    ...
    int64_t GetLastPaid(const PatriotnodeRef& mn, int count_enabled, const CBlockIndex* BlockReading) const;
    int64_t SecondsSincePayment(const PatriotnodeRef& mn, int count_enabled, const CBlockIndex* BlockReading) const;
