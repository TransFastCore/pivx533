...
void CNode::copyStats(CNodeStats& stats, const std::vector<bool>& m_asmap)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (TrumpCoin users should be well used to small numbers with many decimal places by now :)
...
void CConnman::RelayInv(CInv& inv)
{
    ...
    for (CNode* pnode : vNodes){
        ...
        if ((pnode->nServices == NODE_BLOOM_WITHOUT_PN) && inv.IsPatriotNodeType()) continue;
...
bool validatePatriotnodeIP(const std::string& addrStr)
{
    CNetAddr resolved;
