...
CAmount CWalletTx::GetLockedCredit() const
{
    ...
    for (unsigned int i = 0; i < tx->vout.size(); i++) {
        ...
        // Add patriotnode collaterals which are handled like locked coins
        else if (fPatriotNode && tx->vout[i].nValue == collAmt) {
...
bool CWallet::GetPatriotnodeVinAndKeys(CTxIn& txinRet, CPubKey& pubKeyRet, CKey& keyRet, std::string strTxHash, std::string strOutputIndex, std::string& strError)
{
    // wait for reindex and/or import to finish
    if (fImporting || fReindex) return false;

    if (strTxHash.empty() || strOutputIndex.empty()) {
        strError = "Invalid patriotnode collateral hash or output index";
    ...
    try {
        ...
        strError = "Invalid patriotnode output index";
    ...
    // Patriotnode collateral value
    if (txOut.nValue != Params().GetConsensus().nPNCollateralAmt) {
        strError = strprintf("Invalid collateral tx value, must be %s TRUMP", FormatMoney(Params().GetConsensus().nPNCollateralAmt));
        return error("%s: tx %s, index %d not a patriotnode collateral", __func__, strTxHash, nOutputIndex);
    }
    ...
    // Depth must be at least PATRIOTNODE_MIN_CONFIRMATIONS
    if (nDepth < PatriotnodeCollateralMinConf()) {
        strError = strprintf("Collateral tx must have at least %d confirmations, has %d", PatriotnodeCollateralMinConf(), nDepth);
        return error("%s: %s", __func__, strError);
    }
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true) {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-trumpcoin-address
...
void CWallet::AutoCombineDust(CConnman* connman)
{
    ...
    for (const auto& entry : mapCoinsByAddress) {
        ...
        //find patriotnode rewards that need to be combined
...
// Called during Init
void CWallet::ScanPatriotnodeCollateralsAndLock(const CDeterministicPNList& mnList)
{
    LOCK(cs_wallet);

    LogPrintf("Locking patriotnode collaterals...\n");
...
CWallet* CWallet::CreateWalletFromFile(const std::string& name, const fs::path& path)
{
    ...
    if (gArgs.GetBoolArg("-upgradewallet", fFirstRun && !fLegacyWallet) ||
        ...
        if (prev_version <= FEATURE_PRE_TrumpCoin && walletInstance->IsLocked()) {
    ...
    if (fFirstRun) {
        if (!fLegacyWallet) {
            ...
            walletInstance->SetMaxVersion(FEATURE_PRE_TrumpCoin);
        }
