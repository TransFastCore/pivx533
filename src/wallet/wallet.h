...
#ifndef TrumpCoin_WALLET_H
#define TrumpCoin_WALLET_H
...
enum WalletFeature {
    ...
    FEATURE_PRE_TrumpCoin = 61000, // inherited version..
...
public:
     ...
     * TrumpCoin: group coins by pair <CTxDestination, Optional<CTxDestination>>. The optional destination
    ...
    bool GetPatriotnodeVinAndKeys(CTxIn& txinRet, CPubKey& pubKeyRet,
    ...
    /*
     *  Locks cs_wallet
     *  Called during Init. If a DPN collateral is found in the wallet,
     *  lock the corresponding coin, to prevent accidental spending.
     */
    void ScanPatriotnodeCollateralsAndLock(const CDeterministicPNList& mnList);
