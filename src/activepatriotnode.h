...
public:
    ...
    OperationResult GetOperatorKey(CBLSSecretKey& key, CDeterministicPNCPtr& dmn) const;
...
{
private:
    int status{ACTIVE_PATRIOTNODE_INITIAL};
    std::string notCapableReason;

public:
    CActivePatriotnode() = default;
...
// Compatibility code: get vin and keys for either legacy or deterministic patriotnode
bool GetActivePatriotnodeKeys(CTxIn& vin, Optional<CKey>& key, CBLSSecretKey& blsKey);
// Get active patriotnode BLS operator keys for DPN
