...
struct ValidationInterfaceConnections {
    ...
    boost::signals2::scoped_connection NotifyPatriotnodeListChanged;
};
...
struct MainSignalsInstance {
    ...
    /** Notifies listeners of updated deterministic patriotnode list */
    boost::signals2::signal<void (bool undo, const CDeterministicPNList& oldPNList, const CDeterministicPNListDiff& diff)> NotifyPatriotnodeListChanged;
...
void RegisterValidationInterface(CValidationInterface* pwalletIn)
{
    ...
    conns.NotifyPatriotnodeListChanged = g_signals.m_internals->NotifyPatriotnodeListChanged.connect(std::bind(&CValidationInterface::NotifyPatriotnodeListChanged, pwalletIn, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
}
...
void CMainSignals::NotifyPatriotnodeListChanged(bool undo, const CDeterministicPNList& oldPNList, const CDeterministicPNListDiff& diff) {
    m_internals->NotifyPatriotnodeListChanged(undo, oldPNList, diff);
    LOG_EVENT("%s: (undo=%d) old list for=%s, added=%d, updated=%d, removed=%d", __func__,
              undo,
              oldPNList.GetBlockHash().ToString(),
              diff.addedPNs.size(),
              diff.updatedPNs.size(),
