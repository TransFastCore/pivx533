...
bool CheckProRegTx(const CTransaction& tx, const CBlockIndex* pindexPrev, CValidationState& state)
{
    ...
    // It's allowed to set addr to 0, which will put the PN into PoSe-banned state and require a ProUpServTx to be issues later
    ...
    if (pindexPrev) {
        auto mnList = deterministicPNManager->GetListForBlock(pindexPrev);
        // only allow reusing of addresses when it's for the same collateral (which replaces the old PN)
        if (mnList.HasUniqueProperty(pl.addr) && mnList.GetUniquePropertyPN(pl.addr)->collateralOutpoint != pl.collateralOutpoint) {
            return state.DoS(10, false, REJECT_DUPLICATE, "bad-protx-dup-IP-address");
        }
        // never allow duplicate keys, even if this ProTx would replace an existing PN
...
bool CheckProUpServTx(const CTransaction& tx, const CBlockIndex* pindexPrev, CValidationState& state)
{
    ...
    if (pindexPrev) {
        auto mnList = deterministicPNManager->GetListForBlock(pindexPrev);
        auto mn = mnList.GetPN(pl.proTxHash);
        if (!mn) {
            return state.DoS(100, false, REJECT_INVALID, "bad-protx-hash");
        }

        // don't allow updating to addresses already used by other PNs
        if (mnList.HasUniqueProperty(pl.addr) && mnList.GetUniquePropertyPN(pl.addr)->proTxHash != pl.proTxHash) {
        ...
        // we can only check the signature if pindexPrev != nullptr and the PN is known
...
bool CheckProUpRegTx(const CTransaction& tx, const CBlockIndex* pindexPrev, CValidationState& state)
{
    ...
    if (pindexPrev) {
        ...
        auto mnList = deterministicPNManager->GetListForBlock(pindexPrev);
        auto dmn = mnList.GetPN(pl.proTxHash);
        if (!dmn) {
            return state.DoS(100, false, REJECT_INVALID, "bad-protx-hash");
        }
        ...
        if (mnList.HasUniqueProperty(pl.pubKeyOperator)) {
            auto otherDmn = mnList.GetUniquePropertyPN(pl.pubKeyOperator);
