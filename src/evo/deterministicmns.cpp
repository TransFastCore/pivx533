...
std::string CDeterministicPNState::ToString() const
{
    ...
    return strprintf("CDeterministicPNState(nRegisteredHeight=%d, nLastPaidHeight=%d, nPoSePenalty=%d, nPoSeRevivedHeight=%d, nPoSeBanHeight=%d, nRevocationReason=%d, ownerAddress=%s, operatorPubKey=%s, votingAddress=%s, addr=%s, payoutAddress=%s, operatorPayoutAddress=%s)",
...
CDeterministicPNCPtr CDeterministicPNList::GetPNByOperatorKey(const CBLSPublicKey& pubKey)
{
...
void CDeterministicPNList::AddPN(const CDeterministicPNCPtr& dmn, bool fBumpTotalCount)
{
    ...
    if (HasUniqueProperty(dmn->pdmnState->keyIDOwner) || HasUniqueProperty(dmn->pdmnState->pubKeyOperator)) {
        throw(std::runtime_error(strprintf("%s: can't add a patriotnode with a duplicate key (%s or %s)", __func__, EncodeDestination(dmn->pdmnState->keyIDOwner), dmn->pdmnState->pubKeyOperator.Get().ToString())));
    }
...
bool CDeterministicPNManager::BuildNewListFromBlock(const CBlock& block, const CBlockIndex* pindexPrev, CValidationState& _state, CDeterministicPNList& mnListRet, bool debugLogs)
{
    ...
    for (int i = 1; i < (int)block.vtx.size(); i++) {
        ...
        if (tx.nType == CTransaction::TxType::PROREG) {
            ...
            if (newList.HasUniqueProperty(pl.pubKeyOperator) && newList.GetUniquePropertyPN(pl.pubKeyOperator)->proTxHash != pl.proTxHash) {
                return _state.DoS(100, false, REJECT_DUPLICATE, "bad-protx-dup-operator-key");
            }
            auto newState = std::make_shared<CDeterministicPNState>(*dmn->pdmnState);
