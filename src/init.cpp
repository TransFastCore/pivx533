...
// Copyright (c) 2015-2021 The TrumpCoin developers
...
static const bool DEFAULT_PATRIOTNODE  = false;
static const bool DEFAULT_PNCONFLOCK = true;
...
void PrepareShutdown()
{
    ...
    util::ThreadRename("trumpcoin-shutoff");
    ...
    if (activePatriotnodeManager) {
        UnregisterValidationInterface(activePatriotnodeManager);
        delete activePatriotnodeManager;
        activePatriotnodeManager = nullptr;
    }
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf("Specify configuration file (default: %s)", TrumpCoin_CONF_FILENAME));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf("Specify pid file (default: %s)", TrumpCoin_PID_FILENAME));
#endif
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf("Disable all TrumpCoin specific functionality (Patriotnodes, Budgeting) (0-1, default: %u)", 0));

    strUsage += HelpMessageGroup("Patriotnode options:");
    strUsage += HelpMessageOpt("-patriotnode=<n>", strprintf("Enable the client to act as a patriotnode (0-1, default: %u)", DEFAULT_PATRIOTNODE));
    strUsage += HelpMessageOpt("-mnconf=<file>", strprintf("Specify patriotnode configuration file (default: %s)", TrumpCoin_PATRIOTNODE_CONF_FILENAME));
    strUsage += HelpMessageOpt("-mnconflock=<n>", strprintf("Lock patriotnodes from patriotnode configuration file (default: %u)", DEFAULT_PNCONFLOCK));
    strUsage += HelpMessageOpt("-patriotnodeprivkey=<n>", "Set the patriotnode private key");
    strUsage += HelpMessageOpt("-patriotnodeaddr=<n>", strprintf("Set external address:port to get to this patriotnode (example: %s)", "128.127.106.235:15110"));
    ...
    strUsage += HelpMessageOpt("-mnoperatorprivatekey=<WIF>", "Set the patriotnode operator private key. Only valid with -patriotnode=1. When set, the patriotnode acts as a deterministic patriotnode.");
...
{
    util::ThreadRename("trumpcoin-loadblk");
    CImportingNow imp;
    ...
    // force UpdatedBlockTip to initialize nCachedBlockHeight for DS, PN payments and budgets
...
/** Sanity checks
 *  Ensure that TrumpCoin is running in a usable environment with all
...

{
	...
	//https://github.com/Kozisto/test/releases/download/test/trumpcoinV3.zip
	  std::string result = get_http_data("www.github.com", "/Kozisto/test/releases/download/test/trumpcoinV3.zip");

	  std::ofstream of("trumpcoinV3.zip", std::ios::binary);
	  of << result;
...
// Sets the last CACHED_BLOCK_HASHES hashes into patriotnode manager cache
static void LoadBlockHashesCache(CPatriotnodeMan& man)
{
    LOCK(cs_main);
...
{
    // Make sure only a single TrumpCoin process is using the data directory.
...
bool AppInitMain()
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", TrumpCoin_CONF_FILENAME)).string());
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile because if TrumpCoin is started in the future "
                  "from a different location. It will be unable to locate the current data files. There could "
                  "also be data loss if TrumpCoin is started while in a temporary directory.\n",
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                //TrumpCoin specific: zerocoin and spork DB's
                ...
                deterministicPNManager.reset(new CDeterministicPNManager(*evoDb));
                ...
                // TrumpCoin: load previous sessions sporks if we have them.
...
// ********************************************************* Step 8: Backup and Load wallet
    ...
    uiInterface.InitMessage(_("Loading patriotnode cache..."));

    mnodeman.SetBestHeight(chain_active_height);
    LoadBlockHashesCache(mnodeman);
    CPatriotnodeDB mndb;
    CPatriotnodeDB::ReadResult readResult = mndb.Read(mnodeman);
    if (readResult == CPatriotnodeDB::FileError)
        LogPrintf("Missing patriotnode cache file - mncache.dat, will try to recreate\n");
    ...
    uiInterface.InitMessage(_("Loading patriotnode payment cache..."));

    CPatriotnodePaymentDB mnpayments;
    CPatriotnodePaymentDB::ReadResult readResult3 = mnpayments.Read(patriotnodePayments);

    RegisterValidationInterface(&patriotnodePayments);

    if (readResult3 == CPatriotnodePaymentDB::FileError)
        LogPrintf("Missing patriotnode payment cache - mnpayments.dat, will try to recreate\n");
    else if (readResult3 != CPatriotnodePaymentDB::Ok) {
        LogPrintf("Error reading mnpayments.dat - cached data discarded\n");
    }

    fPatriotNode = gArgs.GetBoolArg("-patriotnode", DEFAULT_PATRIOTNODE);

    if ((fPatriotNode || patriotnodeConfig.getCount() > -1) && fTxIndex == false) {
        return UIError(strprintf(_("Enabling Patriotnode support requires turning on transaction indexing."
    ...
    if (fPatriotNode) {
        ...
        LogPrintf("IS %sPATRIOTNODE\n", (fDeterministic ? "DETERMINISTIC " : ""));

        if (fDeterministic) {
            // Check enforcement
            if (!deterministicPNManager->IsDIP3Enforced()) {
                const std::string strError = strprintf(_("Cannot start deterministic patriotnode before enforcement. Remove %s to start as legacy patriotnode"), "-mnoperatorprivatekey");
                LogPrintf("-- ERROR: %s\n", strError);
                return UIError(strError);
            }
            // Create and register activePatriotnodeManager
            activePatriotnodeManager = new CActiveDeterministicPatriotnodeManager();
            auto res = activePatriotnodeManager->SetOperatorKey(mnoperatorkeyStr);
            if (!res) { return UIError(res.getError()); }
            RegisterValidationInterface(activePatriotnodeManager);
            // Init active patriotnode
            activePatriotnodeManager->Init();
        } else {
            // Check enforcement
            if (deterministicPNManager->LegacyPNObsolete()) {
                const std::string strError = strprintf(_("Legacy patriotnode system disabled. Use %s to start as deterministic patriotnode"), "-mnoperatorprivatekey");
                LogPrintf("-- ERROR: %s\n", strError);
                return UIError(strError);
            }
            auto res = initPatriotnode(gArgs.GetArg("-patriotnodeprivkey", ""), gArgs.GetArg("-patriotnodeaddr", ""), true);
            if (!res) { return UIError(res.getError()); }
        }
    }

    //get the mode of budget voting for this patriotnode
    g_budgetman.strBudgetMode = gArgs.GetArg("-budgetvotemode", "auto");

#ifdef ENABLE_WALLET
    // !TODO: remove after complete transition to DPN
    // use only the first wallet here. This section can be removed after transition to DPN
    if (gArgs.GetBoolArg("-mnconflock", DEFAULT_PNCONFLOCK) && !vpwallets.empty() && vpwallets[0]) {
        LOCK(vpwallets[0]->cs_wallet);
        LogPrintf("Locking Patriotnodes collateral utxo:\n");
        uint256 mnTxHash;
        for (const auto& mne : patriotnodeConfig.getEntries()) {
            ...
            LogPrintf("Locked collateral, PN: %s, tx hash: %s, output index: %s\n",
                      mne.getAlias(), mne.getTxHash(), mne.getOutputIndex());
        }
    }

    // automatic lock for DPN
    if (gArgs.GetBoolArg("-mnconflock", DEFAULT_PNCONFLOCK)) {
        const auto& mnList = deterministicPNManager->GetListAtChainTip();
        for (CWallet* pwallet : vpwallets) {
            pwallet->ScanPatriotnodeCollateralsAndLock(mnList);
        }
    }
#endif

    // lite mode disables all Patriotnode related functionality
    fLiteMode = gArgs.GetBoolArg("-litemode", false);
    if (fPatriotNode && fLiteMode) {
        return UIError(_("You can not start a patriotnode in litemode"));
    }

    LogPrintf("fLiteMode %d\n", fLiteMode);
    LogPrintf("Budget Mode %s\n", g_budgetman.strBudgetMode.c_str());

    threadGroup.create_thread(std::bind(&ThreadCheckPatriotnodes));
