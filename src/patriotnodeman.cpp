...
int CPatriotnodeMan::CheckAndRemove(bool forceExpiredRemoval)
{
    ...
    while (it != mapPatriotnodes.end()) {
        ...
        if (activeState == CPatriotnode::PATRIOTNODE_REMOVE ||
            ...
            // clean PN pings right away.
            auto itPing = mapSeenPatriotnodePing.begin();
            while (itPing != mapSeenPatriotnodePing.end()) {
                if (itPing->second.GetVin().prevout == it->first) {
                    itPing = mapSeenPatriotnodePing.erase(itPing);
                } else {
                    ++itPing;
                }
            }
...
CPatriotnodeMan::PNsInfo CPatriotnodeMan::getPNsInfo() const
{
    PNsInfo info;
    int nMinProtocol = ActiveProtocol();
    bool spork_8_active = sporkManager.IsSporkActive(SPORK_8_PATRIOTNODE_PAYMENT_ENFORCEMENT);
    ...
    // legacy patriotnodes
    {
        ...
        for (const auto& it : mapPatriotnodes) {
            ...
            if (spork_8_active && (GetAdjustedTime() - mn->sigTime < PN_WINNER_MINIMUM_AGE)) {
...
int CPatriotnodeMan::CountEnabled(bool only_legacy) const
{
    int count_enabled = 0;
    ...
    if (!only_legacy && deterministicPNManager->IsDIP3Enforced()) {
        count_enabled += deterministicPNManager->GetListAtChainTip().GetValidPNsCount();
    }
    ...
    */
    ...
    int count_enabled = CountEnabled();
    {
        ...
        for (const auto& it : mapPatriotnodes) {
            ...
            if (canSchedulePN(fFilterSigTime, it.second, minProtocol, count_enabled, nBlockHeight)) {
                vecPatriotnodeLastPaid.emplace_back(SecondsSincePayment(it.second, count_enabled, BlockReading), it.second);
            }
        }
    }
    ...
    if (deterministicPNManager->IsDIP3Enforced()) {
        ...
        mnList.ForEachPN(true, [&](const CDeterministicPNCPtr& dmn) {
            ...
            if (canSchedulePN(fFilterSigTime, mn, minProtocol, count_enabled, nBlockHeight)) {
                vecPatriotnodeLastPaid.emplace_back(SecondsSincePayment(mn, count_enabled, BlockReading), mn);
            }
        });
    }
    ...
    if (fFilterSigTime && nCount < count_enabled / 3) return GetNextPatriotnodeInQueueForPayment(nBlockHeight, false, nCount, BlockReading);
...
bool CPatriotnodeMan::CheckInputs(CPatriotnodeBroadcast& mnb, int nChainHeight, int& nDoS)
{
    ...
    if (collateralUtxo.out.nValue != Params().GetConsensus().nPNCollateralAmt) {
        LogPrint(BCLog::PATRIOTNODE,"mnb - invalid amount for mnb collateral %s\n", mnb.vin.prevout.ToString());
        nDoS = 33;
        return false;
    }
    ...
    if (collateralUtxo.out.scriptPubKey != payee) {
        LogPrint(BCLog::PATRIOTNODE,"mnb - collateral %s not associated with mnb pubkey\n", mnb.vin.prevout.ToString());
        nDoS = 33;
        return false;
    }
    ...
    if (collateralUtxoDepth < PatriotnodeCollateralMinConf()) {
        ...
        mapSeenPatriotnodeBroadcast.erase(mnb.GetHash());
        patriotnodeSync.mapSeenSyncPNB.erase(mnb.GetHash());
        return false;
    }
    ...
    if (pConfIndex->GetBlockTime() > mnb.sigTime) {
        LogPrint(BCLog::PATRIOTNODE,"mnb - Bad sigTime %d for Patriotnode %s (%i conf block is at %d)\n",
                 mnb.sigTime, mnb.vin.prevout.hash.ToString(), PatriotnodeCollateralMinConf(), pConfIndex->GetBlockTime());
        return false;
    }
...
int CPatriotnodeMan::ProcessPNBroadcast(CNode* pfrom, CPatriotnodeBroadcast& mnb)
{
    ...
    LogPrint(BCLog::PATRIOTNODE,"%s - Got NEW Patriotnode entry - %s - %lli \n", __func__,
             mnb.vin.prevout.hash.ToString(), mnb.sigTime);
    CPatriotnode mn(mnb);
    if (!Add(mn)) {
        LogPrint(BCLog::PATRIOTNODE, "%s - Rejected Patriotnode entry %s\n", __func__,
                 mnb.vin.prevout.hash.ToString());
        return 0;
    }

    // if it matches our PN pubkey, then we've been remotely activated
    if (mnb.pubKeyPatriotnode == activePatriotnode.pubKeyPatriotnode && mnb.protocolVersion == PROTOCOL_VERSION) {
    ...
    if (!isLocal && patriotnodeSync.IsSynced()) mnb.Relay();
...
int64_t CPatriotnodeMan::SecondsSincePayment(const PatriotnodeRef& mn, int count_enabled, const CBlockIndex* BlockReading) const
{
...
int64_t CPatriotnodeMan::GetLastPaid(const PatriotnodeRef& mn, int count_enabled, const CBlockIndex* BlockReading) const
{
