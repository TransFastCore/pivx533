...
QString ClientModel::getPatriotnodeCountString() const
{
    const auto& info = mnodeman.getPNsInfo();
...
QString ClientModel::getPatriotnodesCount()
{
    if (!cachedPatriotnodeCountString.isEmpty()) {
        return cachedPatriotnodeCountString;
    }

    // Force an update
    cachedPatriotnodeCountString = getPatriotnodeCountString();
    return cachedPatriotnodeCountString;
}

CAmount ClientModel::getPNCollateralRequiredAmount()
{
    return Params().GetConsensus().nPNCollateralAmt;
}
...
void ClientModel::updateMnTimer()
{
    ...
    QString newPatriotnodeCountString = getPatriotnodeCountString();

    if (cachedPatriotnodeCountString != newPatriotnodeCountString) {
        cachedPatriotnodeCountString = newPatriotnodeCountString;

        Q_EMIT strPatriotnodesChanged(cachedPatriotnodeCountString);
    }
}

void ClientModel::startPatriotnodesTimer()
{
...
void ClientModel::stopPatriotnodesTimer()
{
    if (pollMnTimer->isActive()) {
        pollMnTimer->stop();
    }
