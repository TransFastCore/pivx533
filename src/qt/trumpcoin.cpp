...
#include "qt/trumpcoin/splash.h"
#include "qt/trumpcoin/welcomecontentwidget.h"
#include "utilitydialog.h"
#include "winshutdownmonitor.h"

#ifdef ENABLE_WALLET
#include "qt/trumpcoin/governancemodel.h"
...
{
    return QCoreApplication::translate("trumpcoin-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in trumpcoin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in trumpcoin.qrc)
...
/** Class encapsulating TrumpCoin Core startup and shutdown.
...
/** Main TrumpCoin application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (TrumpCoinGUI)
...
{
    window = new TrumpCoinGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &TrumpCoinGUI::detectShutdown);
}
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &TrumpCoinGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (!vpwallets.empty()) {
            ...
            window->addWallet(TrumpCoinGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(TrumpCoinGUI::DEFAULT_WALLET);
            window->setPNModel(mnModel);
        }
#endif
        ...
        // TrumpCoin: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &TrumpCoinGUI::handlePaymentRequest);
        connect(window, &TrumpCoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. TrumpCoin can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(trumpcoin_locale);
    Q_INIT_RESOURCE(trumpcoin);
    ...
    /// 6. Determine availability of data and blocks directory and parse trumpcoin.conf
    ...
    try {
        gArgs.ReadConfigFile(gArgs.GetArg("-conf", TrumpCoin_CONF_FILENAME));
    } catch (const std::exception& e) {
        QMessageBox::critical(nullptr, PACKAGE_NAME,
...
#ifdef ENABLE_WALLET
    /// 7a. parse patriotnode.conf
    std::string strErr;
    if (!patriotnodeConfig.read(strErr)) {
        QMessageBox::critical(nullptr, PACKAGE_NAME,
            QObject::tr("Error reading patriotnode configuration file: %1").arg(strErr.c_str()));
        return 0;
    }
    ...
    // trumpcoin: links repeatedly have their payment requests routed to this process:
