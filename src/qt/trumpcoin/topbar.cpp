...
#include "qt/trumpcoin/forms/ui_topbar.h"
#include "qt/trumpcoin/lockunlock.h"
#include "qt/trumpcoin/qtutils.h"
#include "qt/trumpcoin/receivedialog.h"
#include "qt/trumpcoin/loadingdialog.h"
#include "askpassphrasedialog.h"

#include "bitcoinunits.h"
#include "qt/trumpcoin/balancebubble.h"
...
TopBar::TopBar(TrumpCoinGUI* _mainWindow, QWidget *parent) :
...
void TopBar::setNumBlocks(int count)
{
    ...
    std::string text;
    bool needState = true;
    if (patriotnodeSync.IsBlockchainSyncedReadOnly()) {
        // chain synced
        ...
        if (patriotnodeSync.IsSynced()) {
            ...
            int nAttempt = patriotnodeSync.RequestedPatriotnodeAttempt < PATRIOTNODE_SYNC_THRESHOLD ?
                       patriotnodeSync.RequestedPatriotnodeAttempt + 1 :
                       PATRIOTNODE_SYNC_THRESHOLD;
            int progress = nAttempt + (patriotnodeSync.RequestedPatriotnodeAssets - 1) * PATRIOTNODE_SYNC_THRESHOLD;
            if (progress >= 0) {
                // todo: PN progress..
                text = strprintf("%s - Block: %d", patriotnodeSync.GetSyncStatus(), count);
                //progressBar->setMaximum(4 * PATRIOTNODE_SYNC_THRESHOLD);
...
void TopBar::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("TrumpCoin")
    updateDisplayUnit();
