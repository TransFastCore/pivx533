...
void PatriotNodesWidget::setPNModel(PNModel* _mnModel)
{
    mnModel = _mnModel;
...
static bool startPN(const CPatriotnodeConfig::CPatriotnodeEntry& mne, int chainHeight, std::string& strError)
{
    CPatriotnodeBroadcast mnb;
    if (!CPatriotnodeBroadcast::Create(mne.getIp(), mne.getPrivKey(), mne.getTxHash(), mne.getOutputIndex(), strError, mnb, false, chainHeight))
        return false;
...
void PatriotNodesWidget::startAlias(const QString& strAlias)
{
    ...
    for (const auto& mne : patriotnodeConfig.getEntries()) {
        if (mne.getAlias() == strAlias.toStdString()) {
            std::string strError;
            strStatusHtml += (!startPN(mne, walletModel->getLastBlockProcessedNum(), strError)) ? ("failed to start.\nError: " + QString::fromStdString(strError)) : "successfully started.";
            break;
        }
    }
...
bool PatriotNodesWidget::startAll(QString& failText, bool onlyMissing)
{
    ...
    for (const auto& mne : patriotnodeConfig.getEntries()) {
        ...
        std::string strError;
        if (!startPN(mne, walletModel->getLastBlockProcessedNum(), strError)) {
            amountOfMnFailed++;
        } else {
            amountOfMnStarted++;
        }
