...
GovernanceModel::GovernanceModel(ClientModel* _clientModel, PNModel* _mnModel) : clientModel(_clientModel), mnModel(_mnModel) {}
...
std::vector<VoteInfo> GovernanceModel::getLocalPNsVotesForProposal(const ProposalInfo& propInfo)
{
    // First, get the local patriotnodes
    std::vector<std::pair<COutPoint, std::string>> vecLocalMn;
    for (int i = 0; i < mnModel->rowCount(); ++i) {
        vecLocalMn.emplace_back(std::make_pair(
                COutPoint(uint256S(mnModel->index(i, PNModel::COLLATERAL_ID, QModelIndex()).data().toString().toStdString()),
                mnModel->index(i, PNModel::COLLATERAL_OUT_INDEX, QModelIndex()).data().toInt()),
                mnModel->index(i, PNModel::ALIAS, QModelIndex()).data().toString().toStdString())
        );
    }
    ...
    std::vector<VoteInfo> localVotes;
    {
        ...
        // Get the budget proposal, get the votes, then loop over it and return the ones that correspond to the local patriotnodes here.
...
OperationResult GovernanceModel::validatePropAmount(CAmount amount) const
{
    if (amount < PROPOSAL_MIN_AMOUNT) { // Future: move constant to a budget interface.
        return {false, strprintf("Amount below the minimum of %s TRUMP", FormatMoney(PROPOSAL_MIN_AMOUNT))};
    }

    if (amount > getMaxAvailableBudgetAmount()) {
        return {false, strprintf("Amount exceeding the maximum available of %s TRUMP", FormatMoney(getMaxAvailableBudgetAmount()))};
    }
    return {true};
}
...
{
    return patriotnodeSync.IsBlockchainSynced();
}
                                                 ...
                                                 const std::vector<std::string>& mnVotingAlias)
{
    ...
    for (const auto& mnAlias : mnVotingAlias) {
        bool fLegacyPN = true; // For now, only legacy PNs
