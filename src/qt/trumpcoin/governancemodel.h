...
public:
    ...
    /** Amount of TRUMP paid per month */
    CAmount amount;
...
public:
    explicit GovernanceModel(ClientModel* _clientModel, PNModel* _mnModel);
    ...
    // Return the minimum time when an PN can update a vote for a proposal
    ...
    // Return the votes that the local patriotnodes did for the inputted proposal
    std::vector<VoteInfo> getLocalPNsVotesForProposal(const ProposalInfo& propInfo);
