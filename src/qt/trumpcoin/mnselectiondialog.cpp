...
#include "qt/trumpcoin/forms/ui_mnselectiondialog.h"
...
void MnSelectionDialog::setModel(PNModel* _mnModel, int _minVoteUpdateTimeInSecs)
{
    mnModel = _mnModel;
...
void MnSelectionDialog::updateView()
{
    ...
    for (int i = 0; i < mnModel->rowCount(); ++i) {
        QString alias = mnModel->index(i, PNModel::ALIAS, QModelIndex()).data().toString();
        QString status = mnModel->index(i, PNModel::STATUS, QModelIndex()).data().toString();
                                   ...
                                   VoteInfo* ptrVoteInfo)
{
    ...
    itemOutput->setToolTip(COLUMN_STATUS, "Patriotnode status"); // future: add status description
    ...
    if (ptrVoteInfo) {
        ...
        itemOutput->setToolTip(COLUMN_VOTE, tr("No vote has been emitted from this Patriotnode"));
    }
    ...
    // Disable PNs that will not be able to vote for the proposal until the minimum vote time passes.
