...
#include "qt/trumpcoin/dashboardwidget.h"
#include "qt/trumpcoin/forms/ui_dashboardwidget.h"
#include "qt/trumpcoin/sendconfirmdialog.h"
#include "qt/trumpcoin/txrow.h"
...
DashboardWidget::DashboardWidget(TrumpCoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquarePN, "square-chart-mn");
    setCssProperty(ui->labelPiv, "text-chart-piv");
    setCssProperty(ui->labelPN, "text-chart-mn");
    ...
    setCssProperty(ui->labelAmountPN, "text-stake-mn-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &TrumpCoinGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("TRUMP")
    updateDisplayUnit();
}

void DashboardWidget::onTxArrived(const QString& hash, const bool isCoinStake, const bool isPNReward, const bool isCSAnyType)
{
    showList();
    if (!isVisible()) return;
#ifdef USE_QTCHARTS
    if (isCoinStake || isPNReward) {
...
QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isPiv = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::PNReward;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair TRUMP, PN Reward
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            chartData->totalPN += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesPiv.append(piv);
        chartData->valuesPN.append(mn);
    ...
    if (fShowCharts) {
        if (!stakesFilter) {
            ...
            stakesFilter->setTypeFilter(TransactionFilterProxy::TYPE(TransactionRecord::StakeMint) |
                                        ...
                                        TransactionFilterProxy::TYPE(TransactionRecord::StakeZTRUMP) |
                                        TransactionFilterProxy::TYPE(TransactionRecord::StakeDelegated) |
                                        TransactionFilterProxy::TYPE(TransactionRecord::PNReward));
        }
