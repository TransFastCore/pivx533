    ...
    clientModel(_clientModel)
{
    ...
    ui->labelMessage3->setText(formatHtmlContent(
                ...
                formatParagraph(tr("The Address is labeled under the PatriotNode's name."))));
...
bool PatriotNodeWizardDialog::createPN()
{
    ...
    if (!walletModel->getPNCollateralCandidate(collateralOut)) {
        ...
        if (prepareStatus.status != WalletModel::OK) {
            returnStr = tr("Prepare PatriotNode failed.\n\n%1\n").arg(returnMsg);
            return false;
        }
    ...
    returnStr = tr("PatriotNode created! Wait %1 confirmations before starting it.").arg(PatriotnodeCollateralMinConf());
    return true;
}
