...
#include "qt/trumpcoin/votedialog.h"
#include "qt/trumpcoin/forms/ui_votedialog.h"

#include "qt/trumpcoin/mnmodel.h"
#include "qt/trumpcoin/mnselectiondialog.h"
#include "qt/trumpcoin/qtutils.h"

VoteDialog::VoteDialog(QWidget *parent, GovernanceModel* _govModel, PNModel* _mnModel) :
    QDialog(parent),
    ...
    mnModel(_mnModel)
{
    ...
    setCssProperty(ui->btnSelectPatriotnodes, "btn-vote-select");
    ...
    connect(ui->btnSelectPatriotnodes, &QPushButton::clicked, this, &VoteDialog::onMnSelectionClicked);
...
void VoteDialog::setProposal(const ProposalInfo& prop)
{
    ...
    votes = govModel->getLocalPNsVotesForProposal(prop);
...
void VoteDialog::onAcceptClicked()
{
    ...
    if (vecSelectedMn.empty()) {
        inform(tr("Missing voting patriotnodes selection"));
        return;
    }
...
{
    TrumpCoinGUI* window = dynamic_cast<TrumpCoinGUI*>(parent());
...
void VoteDialog::updateMnSelectionNum()
{
    QString text;
    if (vecSelectedMn.empty()) {
        text = !votes.empty() ? tr("You have voted with %1 Patriotnodes for this proposal\nChange votes").arg(votes.size()) :
                tr("Select Voting Patriotnodes");
    } else {
        text = tr("%1 Patriotnodes selected to vote").arg(vecSelectedMn.size());
    }
    ui->btnSelectPatriotnodes->setText(text);
}
