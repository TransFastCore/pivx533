...
bool ProcessBlockFound(const std::shared_ptr<const CBlock>& pblock, CWallet& wallet, std::unique_ptr<CReserveKey>& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("TrumpCoinMiner : generated block is stale");
    }
    ...
    if (!res || sc.stateErrorFound()) {
        return error("TrumpCoinMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("TrumpCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("trumpcoin-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running TrumpCoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("TrumpCoinMiner exception");
    } catch (...) {
        LogPrintf("TrumpCoinMiner exception");
    }

    LogPrintf("TrumpCoinMiner exiting\n");
}
