...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    case MSG_PATRIOTNODE_WINNER:
        if (patriotnodePayments.mapPatriotnodePayeeVotes.count(inv.hash)) {
            patriotnodeSync.AddedPatriotnodeWinner(inv.hash);
            return true;
        }
        return false;
    case MSG_BUDGET_VOTE:
        if (g_budgetman.HaveSeenProposalVote(inv.hash)) {
            patriotnodeSync.AddedBudgetItem(inv.hash);
            return true;
        }
        return false;
    ...
    case MSG_BUDGET_FINALIZED:
        ...
        return false;
    case MSG_PATRIOTNODE_ANNOUNCE:
        if (mnodeman.mapSeenPatriotnodeBroadcast.count(inv.hash)) {
            patriotnodeSync.AddedPatriotnodeList(inv.hash);
            return true;
        }
        return false;
    case MSG_PATRIOTNODE_PING:
        return mnodeman.mapSeenPatriotnodePing.count(inv.hash);
    }
                                      ...
                                      int chainHeight)
{
    ...
    // !TODO: remove when transition to DPN is complete
    if (inv.type == MSG_PATRIOTNODE_WINNER && !deterministicPNManager->LegacyPNObsolete()) {
        if (patriotnodePayments.mapPatriotnodePayeeVotes.count(inv.hash)) {
            CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
            ss.reserve(1000);
            ss << patriotnodePayments.mapPatriotnodePayeeVotes[inv.hash];
            connman->PushMessage(pfrom, msgMaker.Make(NetMsgType::PNWINNER, ss));
            return true;
        }
    }
    ...
    // !TODO: remove when transition to DPN is complete
    if (inv.type == MSG_PATRIOTNODE_ANNOUNCE && !deterministicPNManager->LegacyPNObsolete()) {
        auto it = mnodeman.mapSeenPatriotnodeBroadcast.find(inv.hash);
        if (it != mnodeman.mapSeenPatriotnodeBroadcast.end()) {
            ...
            ss.reserve(1000);
            ss << mnb;
            std::string msgType = !mnb.addr.IsAddrV1Compatible() ? NetMsgType::PNBROADCAST2 : NetMsgType::PNBROADCAST;
    ...
    // !TODO: remove when transition to DPN is complete
    if (inv.type == MSG_PATRIOTNODE_PING && !deterministicPNManager->LegacyPNObsolete()) {
        if (mnodeman.mapSeenPatriotnodePing.count(inv.hash)) {
            CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
            ss.reserve(1000);
            ss << mnodeman.mapSeenPatriotnodePing[inv.hash];
            connman->PushMessage(pfrom, msgMaker.Make(NetMsgType::PNPING, ss));
            return true;
        }
    }
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived, CConnman* connman, std::atomic<bool>& interruptMsgProc)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // TrumpCoin: We use certain sporks during IBD, so check to see if they are
    ...
    // getaddr message mitigates the attack.
        ...
        if (std::find(allMessages.begin(), allMessages.end(), strCommand) != allMessages.end()) {
            ...
            if (!patriotnodeSync.MessageDispatcher(pfrom, strCommand, vRecv)) {
                // Probably one the extensions
                mnodeman.ProcessMessage(pfrom, strCommand, vRecv);
                g_budgetman.ProcessMessage(pfrom, strCommand, vRecv);
                patriotnodePayments.ProcessMessagePatriotnodePayments(pfrom, strCommand, vRecv);
                sporkManager.ProcessSpork(pfrom, strCommand, vRecv);
                patriotnodeSync.ProcessMessage(pfrom, strCommand, vRecv);
            }
        } else {
