...
OperationResult CActiveDeterministicPatriotnodeManager::SetOperatorKey(const std::string& strPNOperatorPrivKey)
{
    ...
    if (!info.keyOperator.SetHexStr(strPNOperatorPrivKey)) {
...
OperationResult CActiveDeterministicPatriotnodeManager::GetOperatorKey(CBLSSecretKey& key, CDeterministicPNCPtr& dmn) const
{
    if (!IsReady()) {
...
void CActiveDeterministicPatriotnodeManager::Init()
{
    ...
    CDeterministicPNCPtr dmn = mnList.GetPNByOperatorKey(info.pubKeyOperator);
    if (!dmn) {
...
void CActiveDeterministicPatriotnodeManager::UpdatedBlockTip(const CBlockIndex* pindexNew, const CBlockIndex* pindexFork, bool fInitialDownload)
{
    ...
    if (state == PATRIOTNODE_READY) {
        auto newDmn = deterministicPNManager->GetListForBlock(pindexNew).GetValidPN(info.proTxHash);
        if (newDmn == nullptr) {
        ...
        auto oldDmn = deterministicPNManager->GetListForBlock(pindexNew->pprev).GetPN(info.proTxHash);
        if (oldDmn == nullptr) {
            // should never happen if state is PATRIOTNODE_READY
...
OperationResult initPatriotnode(const std::string& _strPatriotNodePrivKey, const std::string& _strPatriotNodeAddr, bool isFromInit)
{
    ...
    std::string strHost;
    SplitHostPort(_strPatriotNodeAddr, nPort, strHost);
    ...
    if (!addrTest.IsValid()) {
        return errorOut(strprintf(_("Invalid -patriotnodeaddr address: %s"), _strPatriotNodeAddr));
    }
...
void CActivePatriotnode::ManageStatus()
{
    ...
    if (status == ACTIVE_PATRIOTNODE_INITIAL || (pmn && status == ACTIVE_PATRIOTNODE_NOT_CAPABLE)) {
        if (pmn) {
...
bool GetActiveDPNKeys(CBLSSecretKey& key, CTxIn& vin)
{
    if (activePatriotnodeManager == nullptr) {
        return error("%s: Active Patriotnode not initialized", __func__);
    }
    CDeterministicPNCPtr dmn;
    auto res = activePatriotnodeManager->GetOperatorKey(key, dmn);
    if (!res) {
...
bool GetActivePatriotnodeKeys(CTxIn& vin, Optional<CKey>& key, CBLSSecretKey& blsKey)
{
    if (activePatriotnodeManager != nullptr) {
        // deterministic mn
        key = nullopt;
        return GetActiveDPNKeys(blsKey, vin);
    }
    // legacy mn
    ...
    if (activePatriotnode.GetStatus() != ACTIVE_PATRIOTNODE_STARTED) {
        return error("%s: PN not started (%s)", __func__, activePatriotnode.GetStatusMessage());
    }
    vin = *activePatriotnode.vin;
    CKey sk;
    CPubKey pk;
    activePatriotnode.GetKeys(sk, pk);
