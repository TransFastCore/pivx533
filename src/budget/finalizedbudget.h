...
enum class TrxValidationStatus {
    ...
    VoteThreshold    /** If not enough patriotnodes have voted on a finalized budget */
};
...
private:
    ...
    bool fAutoChecked; //If it matches what we see, we'll auto vote for it (patriotnode only)
    bool fValid;
...
public:
    ...
    // Check finalized budget proposals. Patriotnodes only (when voting on finalized budgets)
