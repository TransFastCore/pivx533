...
void CBudgetManager::CheckOrphanVotes()
{
    ...
    LogPrint(BCLog::PNBUDGET,"%s: Done\n", __func__);
}
...
uint256 CBudgetManager::SubmitFinalBudget()
{
    ...
    if (nSubmittedHeight >= nBlockStart){
        LogPrint(BCLog::PNBUDGET,"%s: nSubmittedHeight(=%ld) < nBlockStart(=%ld) condition not fulfilled.\n",
    ...
    if (Params().IsTestnet()) {
        // NOTE: 9 blocks for testnet is way to short to have any patriotnode submit an automatic vote on the finalized(!) budget,
    ...
    if (nBlockStart - nCurrentHeight > finalizationWindow) {
        LogPrint(BCLog::PNBUDGET,"%s: Too early for finalization. Current block is %ld, next Superblock is %ld.\n", __func__, nCurrentHeight, nBlockStart);
        LogPrint(BCLog::PNBUDGET,"%s: First possible block for finalization: %ld. Last possible block for finalization: %ld. "
    ...
    if (vecTxBudgetPayments.size() < 1) {
        LogPrint(BCLog::PNBUDGET,"%s: Found No Proposals For Period\n", __func__);
        return UINT256_ZERO;
    }
    ...
    if (HaveFinalizedBudget(budgetHash)) {
        LogPrint(BCLog::PNBUDGET,"%s: Budget already exists - %s\n", __func__, budgetHash.ToString());
    ...
    if (!mapUnconfirmedFeeTx.count(budgetHash)) {
        ...
        if (vpwallets.empty() || !vpwallets[0]) {
            LogPrint(BCLog::PNBUDGET,"%s: Wallet not found\n", __func__);
            return UINT256_ZERO;
        }
        CReserveKey keyChange(vpwallets[0]);
        if (!vpwallets[0]->CreateBudgetFeeTX(wtx, budgetHash, keyChange, true)) {
            LogPrint(BCLog::PNBUDGET,"%s: Can't make collateral transaction\n", __func__);
            return UINT256_ZERO;
        }
        ...
        if (res.status == CWallet::CommitStatus::OK) {
            ...
            LogPrint(BCLog::PNBUDGET,"%s: Collateral sent. txid: %s\n", __func__, collateraltxid.ToString());
            return budgetHash;
        }
        return UINT256_ZERO;
    }
    ...
    fb.Relay();
    nSubmittedHeight = nCurrentHeight;
    LogPrint(BCLog::PNBUDGET,"%s: Done! %s\n", __func__, budgetHash.ToString());
    return budgetHash;
}
...
std::string CBudgetManager::GetFinalizedBudgetStatus(const uint256& nHash) const
{
    ...
    for (int nBlockHeight = nBlockStart; nBlockHeight <= nBlockEnd; nBlockHeight++) {
        ...
        if (!fb.GetBudgetPaymentByBlock(nBlockHeight, budgetPayment)) {
            LogPrint(BCLog::PNBUDGET,"%s: Couldn't find budget payment for block %lld\n", __func__, nBlockHeight);
            continue;
        }
...
bool CBudgetManager::AddFinalizedBudget(CFinalizedBudget& finalizedBudget, CNode* pfrom)
{
    ...
    if (!CheckCollateral(feeTxId, nHash, strError, finalizedBudget.nTime, nCurrentHeight, true)) {
        LogPrint(BCLog::PNBUDGET,"%s: invalid finalized budget (%s) collateral id=%s - %s\n",
    ...
    if (!finalizedBudget.CheckProposals(mapWinningProposals)) {
        ...
        LogPrint(BCLog::PNBUDGET,"%s: Budget finalization does not match with winning proposals\n", __func__);
    ...
    LogPrint(BCLog::PNBUDGET,"%s: finalized budget %s [%s (%s)] added\n",
...
bool CBudgetManager::AddProposal(CBudgetProposal& budgetProposal)
{
    ...
    if (!CheckCollateral(feeTxId, nHash, strError, budgetProposal.nTime, nCurrentHeight, false)) {
        LogPrint(BCLog::PNBUDGET,"%s: invalid budget proposal (%s) collateral id=%s - %s\n",
                __func__, nHash.ToString(), feeTxId.ToString(), strError);
    ...
    LogPrint(BCLog::PNBUDGET,"%s: budget proposal %s [%s] added\n", __func__, nHash.ToString(), budgetProposal.GetName());
...
void CBudgetManager::CheckAndRemove()
{
    ...
    {
        LOCK(cs_proposals);
        LogPrint(BCLog::PNBUDGET, "%s: mapProposals cleanup - size before: %d\n", __func__, mapProposals.size());
        for (auto& it: mapProposals) {
            ...
            if (!pbudgetProposal->UpdateValid(nCurrentHeight)) {
                ...
                mapFeeTxToProposal.erase(pbudgetProposal->GetFeeTXHash());
                 ...
                 LogPrint(BCLog::PNBUDGET,"%s: Found valid budget proposal: %s %s\n", __func__,
        ...
        LogPrint(BCLog::PNBUDGET, "%s: mapProposals cleanup - size after: %d\n", __func__, mapProposals.size());
    }

    // Then check finalized budgets
    {
        LOCK(cs_budgets);
        LogPrint(BCLog::PNBUDGET, "%s: mapFinalizedBudgets cleanup - size before: %d\n", __func__, mapFinalizedBudgets.size());
        for (auto& it: mapFinalizedBudgets) {
            ...
            if (!pfinalizedBudget->UpdateValid(nCurrentHeight)) {
                ...
                LogPrint(BCLog::PNBUDGET,"%s: Found valid finalized budget: %s %s\n", __func__,
        ...
        LogPrint(BCLog::PNBUDGET, "%s: mapFinalizedBudgets cleanup - size after: %d\n", __func__, mapFinalizedBudgets.size());
    }
    // Patriotnodes vote on valid ones
...
bool CBudgetManager::FillBlockPayee(CMutableTransaction& txCoinbase, CMutableTransaction& txCoinstake, const int nHeight, bool fProofOfStake) const
{
    ...
    // Starting from TrumpCoin v6.0 patriotnode and budgets are paid in the coinbase tx of PoS blocks
    ...
    LogPrint(BCLog::PNBUDGET,"%s: Budget payment to %s for %lld\n", __func__, EncodeDestination(address), nAmount);
    return true;
}

void CBudgetManager::VoteOnFinalizedBudgets()
{
    // function called only from initialized patriotnodes
    if (!fPatriotNode) {
        LogPrint(BCLog::PNBUDGET,"%s: Not a patriotnode\n", __func__);
        return;
    }
    ...
    if (GetRandInt(4) != 0) {
        LogPrint(BCLog::PNBUDGET,"%s: waiting\n", __func__);
        return;
    }
    ...
    std::vector<CBudgetProposal> vBudget = GetBudget();
    if (vBudget.empty()) {
        LogPrint(BCLog::PNBUDGET,"%s: No proposal can be finalized\n", __func__);
        return;
    }
    ...
    for (const uint256& budgetHash: vBudgetHashes) {
        ...
        if (mnKey != nullopt) {
            ...
            // DPN
            if (!vote.Sign(blsKey)) {
                LogPrintf("%s: Failure to sign budget %s with DPN\n", __func__, budgetHash.ToString());
                continue;
            }
        }
        std::string strError = "";
        ...
        LogPrint(BCLog::PNBUDGET, "%s: new finalized budget vote signed: %s\n", __func__, vote.GetHash().ToString());
...
bool CBudgetManager::IsBudgetPaymentBlock(int nBlockHeight, int& nCountThreshold) const
{
    ...
    // threshold for highest finalized budgets (highest vote count - 10% of active patriotnodes)
    nCountThreshold = nHighestCount - (nCountEnabled / 10);
    // reduce the threshold if there are less than 10 enabled patriotnodes
    if (nCountThreshold == nHighestCount) nCountThreshold--;

    LogPrint(BCLog::PNBUDGET,"%s: nHighestCount: %lli, 5%% of Patriotnodes: %lli.\n",
            __func__, nHighestCount, nFivePercent);

    // If budget doesn't have 5% of the network votes, then we should pay a patriotnode instead
...
TrxValidationStatus CBudgetManager::IsTransactionValid(const CTransaction& txNew, const uint256& nBlockHash, int nBlockHeight) const
{
    ...
    bool fThreshold = false;
    {
        ...
        if (highestVotesBudget) {
            ...
            // tx not valid
            LogPrint(BCLog::PNBUDGET, "%s: ignoring budget. Out of range or tx not valid.\n", __func__);
        }
    }

    // If not enough patriotnodes autovoted for any of the finalized budgets or if none of the txs
    // are valid, we should pay a patriotnode instead
...
std::vector<CBudgetProposal> CBudgetManager::GetBudget()
{
    ...
    for (CBudgetProposal* pbudgetProposal: vProposalsOrdered) {
        LogPrint(BCLog::PNBUDGET,"%s: Processing Budget %s\n", __func__, pbudgetProposal->GetName());
        //prop start/end should be inside this period
        if (pbudgetProposal->IsPassing(nBlockStart, nBlockEnd, mnCount)) {
            LogPrint(BCLog::PNBUDGET,"%s:  -   Check 1 passed: valid=%d | %ld <= %ld | %ld >= %ld | Yeas=%d Nays=%d Count=%d | established=%d\n",
            ...
            if (pbudgetProposal->GetAmount() + nBudgetAllocated <= nTotalBudget) {
                ...
                LogPrint(BCLog::PNBUDGET,"%s:  -     Check 2 passed: Budget added\n", __func__);
            } else {
                pbudgetProposal->SetAllotted(0);
                LogPrint(BCLog::PNBUDGET,"%s:  -     Check 2 failed: no amount allotted\n", __func__);
            }

        } else {
            LogPrint(BCLog::PNBUDGET,"%s:  -   Check 1 failed: valid=%d | %ld <= %ld | %ld >= %ld | Yeas=%d Nays=%d Count=%d | established=%d\n",
                    __func__, pbudgetProposal->IsValid(), pbudgetProposal->GetBlockStart(), nBlockStart, pbudgetProposal->GetBlockEnd(),
    ...
    while (it != mapFinalizedBudgets.end()) {
        ...
        if (nBlockHeight >= pfinalizedBudget->GetBlockStart() && nBlockHeight <= pfinalizedBudget->GetBlockEnd()) {
            ...
            if (pfinalizedBudget->GetBudgetPaymentByBlock(nBlockHeight, payment)) {
                ...
                LogPrint(BCLog::PNBUDGET,"%s:  Couldn't find budget payment for block %d\n", __func__, nBlockHeight);
            }
        }
...
{
    AssertLockHeld(cs_proposals);
    LogPrint(BCLog::PNBUDGET, "Cleaning proposal votes for %s. Before: YES=%d, NO=%d\n",
    ...
    while (it != prop->mapVotes.end()) {
        ...
        auto dmn = mnList.GetPNByCollateral(it->first);
        if (dmn) {
            (*it).second.SetValid(!dmn->IsPoSeBanned());
        } else {
            // -- Legacy System (!TODO: remove after enforcement) --
            CPatriotnode* pmn = mnodeman.Find(it->first);
            (*it).second.SetValid(pmn && pmn->IsEnabled());
        }
        ++it;
    }

    LogPrint(BCLog::PNBUDGET, "Cleaned proposal votes for %s. After: YES=%d, NO=%d\n",
            prop->GetName(), prop->GetYeas(), prop->GetNays());
}
...
{
    AssertLockHeld(cs_budgets);
    LogPrint(BCLog::PNBUDGET, "Cleaning finalized budget votes for [%s (%s)]. Before: %d\n",
    ...
    LogPrint(BCLog::PNBUDGET, "Cleaned finalized budget votes for [%s (%s)]. After: %d\n",
            fbud->GetName(), fbud->GetProposalsStr(), fbud->GetVoteCount());
}
...
{
    if (patriotnodeSync.RequestedPatriotnodeAssets <= PATRIOTNODE_SYNC_BUDGET) return;
    ...
    if (patriotnodeSync.IsSynced()) {
        LogPrint(BCLog::PNBUDGET,"%s:  incremental sync started\n", __func__);
    ...
    //remove invalid (from non-active patriotnode) votes once in a while
    LogPrint(BCLog::PNBUDGET,"%s:  askedForSourceProposalOrBudget cleanup - size: %d\n", __func__, askedForSourceProposalOrBudget.size());
    ...
    {
        LOCK(cs_proposals);
        LogPrint(BCLog::PNBUDGET,"%s:  mapProposals cleanup - size: %d\n", __func__, mapProposals.size());
        for (auto& it: mapProposals) {
    ...
    {
        LOCK(cs_budgets);
        LogPrint(BCLog::PNBUDGET,"%s:  mapFinalizedBudgets cleanup - size: %d\n", __func__, mapFinalizedBudgets.size());
        for (auto& it: mapFinalizedBudgets) {
    ...
    LogPrint(BCLog::PNBUDGET,"%s:  PASSED\n", __func__);
}
...
int CBudgetManager::ProcessBudgetVoteSync(const uint256& nProp, CNode* pfrom)
{
    if (nProp.IsNull()) {
        ...
        if (!(pfrom->addr.IsRFC1918() || pfrom->addr.IsLocal())) {
            ...
            if (itLastRequest != mAskedUsForBudgetSync.end() && GetTime() < (*itLastRequest).second) {
                LogPrint(BCLog::PATRIOTNODE, "budgetsync - peer %i already asked for budget sync\n", pfrom->GetId());
    ...
    Sync(pfrom, nProp);
    LogPrint(BCLog::PNBUDGET, "mnvs - Sent Patriotnode votes to peer %i\n", pfrom->GetId());
    return 0;
}
...
int CBudgetManager::ProcessProposal(CBudgetProposal& proposal)
{
    ...
    if (HaveProposal(nHash)) {
        patriotnodeSync.AddedBudgetItem(nHash);
        return 0;
    }
    ...
    if (patriotnodeSync.IsSynced()) proposal.Relay();
    patriotnodeSync.AddedBudgetItem(nHash);

    LogPrint(BCLog::PNBUDGET, "mprop (new) %s\n", nHash.ToString());
...
bool CBudgetManager::ProcessProposalVote(CBudgetVote& vote, CNode* pfrom, CValidationState& state)
{
    ...
    if (HaveSeenProposalVote(voteID)) {
        patriotnodeSync.AddedBudgetItem(voteID);
        return false;
    }

    const CTxIn& voteVin = vote.GetVin();

    // See if this vote was signed with a deterministic patriotnode
    std::string err;
    auto mnList = deterministicPNManager->GetListAtChainTip();
    auto dmn = mnList.GetPNByCollateral(voteVin.prevout);
    if (dmn) {
        ...
        if (dmn->IsPoSeBanned()) {
            err = strprintf("patriotnode (%s) not valid or PoSe banned", mn_protx_id);
        ...
        if (patriotnodeSync.IsSynced()) vote.Relay();
        patriotnodeSync.AddedBudgetItem(voteID);
        LogPrint(BCLog::PNBUDGET, "mvote - new vote (%s) for proposal %s from dmn %s\n",
    ...
    CPatriotnode* pmn = mnodeman.Find(voteVin.prevout);
    if (!pmn) {
        err = strprintf("unknown patriotnode - vin: %s", voteVin.prevout.ToString());
        // Ask for PN only if we finished syncing the PN list.
        if (pfrom && patriotnodeSync.IsPatriotnodeListSynced()) mnodeman.AskForPN(pfrom, voteVin);
        return state.DoS(0, false, REJECT_INVALID, "bad-mvote", false, err);
    }

    AddSeenProposalVote(vote);

    if (!vote.CheckSignature(pmn->pubKeyPatriotnode.GetID())) {
        if (patriotnodeSync.IsSynced()) {
            err = strprintf("signature from patriotnode %s invalid", voteVin.prevout.ToString());
    ...
    if (!pmn->IsEnabled()) {
        return state.DoS(0, false, REJECT_INVALID, "bad-mvote", false, "patriotnode not valid");
    }
    if (!UpdateProposal(vote, pfrom, err)) {
        return state.DoS(0, false, REJECT_INVALID, "bad-mvote", false, err);
    }
...
int CBudgetManager::ProcessFinalizedBudget(CFinalizedBudget& finalbudget, CNode* pfrom)
{
    ...
    if (patriotnodeSync.IsSynced()) finalbudget.Relay();
    patriotnodeSync.AddedBudgetItem(nHash);

    LogPrint(BCLog::PNBUDGET, "fbs (new) %s\n", nHash.ToString());
...
bool CBudgetManager::ProcessFinalizedBudgetVote(CFinalizedBudgetVote& vote, CNode* pfrom, CValidationState& state)
{
    ...
    if (dmn) {
        ...
        if (patriotnodeSync.IsSynced()) vote.Relay();
        patriotnodeSync.AddedBudgetItem(voteID);
        LogPrint(BCLog::PNBUDGET, "fbvote - new vote (%s) for budget %s from dmn %s\n",
    ...
    if (!pmn->IsEnabled()) {
        return state.DoS(0, false, REJECT_INVALID, "bad-fbvote", false, "patriotnode not valid");
    }
    if (!UpdateFinalizedBudget(vote, pfrom, err)) {
        return state.DoS(0, false, REJECT_INVALID, "bad-fbvote", false, err);
    }
    ...
    if (patriotnodeSync.IsSynced()) vote.Relay();
    patriotnodeSync.AddedBudgetItem(voteID);
    LogPrint(BCLog::PNBUDGET, "fbvote - new vote (%s) for budget %s from mn %s\n",
...
void CBudgetManager::Sync(CNode* pfrom, const uint256& nProp, bool fPartial)
{
    ...
    g_connman->PushMessage(pfrom, msgMaker.Make(NetMsgType::SYNCSTATUSCOUNT, PATRIOTNODE_SYNC_BUDGET_PROP, nInvCount));
    LogPrint(BCLog::PNBUDGET, "%s: sent %d items\n", __func__, nInvCount);
    ...
    g_connman->PushMessage(pfrom, msgMaker.Make(NetMsgType::SYNCSTATUSCOUNT, PATRIOTNODE_SYNC_BUDGET_FIN, nInvCount));
    LogPrint(BCLog::PNBUDGET, "%s: sent %d items\n", __func__, nInvCount);
...
bool CBudgetManager::UpdateProposal(const CBudgetVote& vote, CNode* pfrom, std::string& strError)
{
    ...
    if (!mapProposals.count(nProposalHash)) {
        if (pfrom) {
            ...
            if (!patriotnodeSync.IsSynced()) return false;

            LogPrint(BCLog::PNBUDGET,"%s: Unknown proposal %d, asking for source proposal\n", __func__, nProposalHash.ToString());
...
bool CBudgetManager::UpdateFinalizedBudget(CFinalizedBudgetVote& vote, CNode* pfrom, std::string& strError)
{
    ...
    if (!mapFinalizedBudgets.count(nBudgetHash)) {
        if (pfrom) {
            ...
            LogPrint(BCLog::PNBUDGET,"%s: Unknown Finalized Proposal %s, asking for source budget\n", __func__, nBudgetHash.ToString());
    ...
    LogPrint(BCLog::PNBUDGET,"%s: Finalized Proposal %s added\n", __func__, nBudgetHash.ToString());
...
bool CheckCollateral(const uint256& nTxCollateralHash, const uint256& nExpectedHash, std::string& strError, int64_t& nTime, int nCurrentHeight, bool fBudgetFinalization)
{
    ...
    for (const CTxOut &o : txCollateral->vout) {
        ...
        if (fBudgetFinalization) {
            ...
            // Note: there are still old valid budgets out there, but the check for the new 5 TRUMP finalization collateral
            //       will also cover the old 50 TRUMP finalization collateral.
            LogPrint(BCLog::PNBUDGET, "Final Budget: o.scriptPubKey(%s) == findScript(%s) ?\n", HexStr(o.scriptPubKey), HexStr(findScript));
            if (o.scriptPubKey == findScript) {
                LogPrint(BCLog::PNBUDGET, "Final Budget: o.nValue(%ld) >= BUDGET_FEE_TX(%ld) ?\n", o.nValue, BUDGET_FEE_TX);
            ...
            LogPrint(BCLog::PNBUDGET, "Normal Budget: o.scriptPubKey(%s) == findScript(%s) ?\n", HexStr(o.scriptPubKey), HexStr(findScript));
            if (o.scriptPubKey == findScript) {
                LogPrint(BCLog::PNBUDGET, "Normal Budget: o.nValue(%ld) >= PROPOSAL_FEE_TX(%ld) ?\n", o.nValue, PROPOSAL_FEE_TX);
