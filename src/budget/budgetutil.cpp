...
// key, alias and collateral outpoint of a patriotnode. Struct used to sign proposal/budget votes
struct MnKeyData
{
    std::string mnAlias;
...
// Legacy patriotnodes
static mnKeyList getPNKeys(const Optional<std::string>& mnAliasFilter,
                           UniValue& resultsObj, int& failed)
{
    ...
    for (const CPatriotnodeConfig::CPatriotnodeEntry& mne : patriotnodeConfig.getEntries()) {
        ...
        if (!CMessageSigner::GetKeysFromSecret(mne.getPrivKey(), mnKey, mnPubKey)) {
            resultsObj.push_back(packErrorRetStatus(mnAlias, "Could not get key from patriotnode.conf"));
            failed++;
            continue;
        }
        CPatriotnode* pmn = mnodeman.Find(mnPubKey);
        if (!pmn) {
            resultsObj.push_back(packErrorRetStatus(mnAlias, "Can't find patriotnode by pubkey"));
            failed++;
            continue;
        }
...
static mnKeyList getPNKeysForActivePatriotnode(UniValue& resultsObj)
{
    // local node must be a patriotnode
    if (!fPatriotNode) {
        throw std::runtime_error(_("This is not a patriotnode. 'local' option disabled."));
    }

    if (activePatriotnode.vin == nullopt) {
        throw std::runtime_error(_("Active Patriotnode not initialized."));
    }

    CKey mnKey; CPubKey mnPubKey;
    activePatriotnode.GetKeys(mnKey, mnPubKey);
    CPatriotnode* pmn = mnodeman.Find(mnPubKey);
    if (!pmn) {
        resultsObj.push_back(packErrorRetStatus("local", "Can't find patriotnode by pubkey"));
        return mnKeyList();
    }
...
static mnKeyList getDPNVotingKeys(CWallet* const pwallet, const Optional<std::string>& mnAliasFilter, bool fFinal, UniValue& resultsObj, int& failed)
{
    if (!pwallet) {
    ...
    CDeterministicPNCPtr mnFilter{nullptr};
    if (mnAliasFilter) {
        // vote with a single patriotnode (identified by ProTx)
        const uint256& proTxHash = uint256S(*mnAliasFilter);
        mnFilter = mnList.GetValidPN(proTxHash);
        if (!mnFilter) {
    ...
    mnList.ForEachPN(true, [&](const CDeterministicPNCPtr& dmn) {
        ...
        if (!mnFilter || filtered) {
            if (fFinal) {
                ...
                throw std::runtime_error("Finalized budget voting is allowed only locally, from the patriotnode");
            }
...
static mnKeyList getDPNKeysForActivePatriotnode(UniValue& resultsObj)
{
    // local node must be a patriotnode
    if (!activePatriotnodeManager) {
        throw std::runtime_error(_("This is not a deterministic patriotnode. 'local' option disabled."));
    }

    CBLSSecretKey sk; CDeterministicPNCPtr dmn;
    auto res = activePatriotnodeManager->GetOperatorKey(sk, dmn);
    if (!res) {
...
// Note: for DPNs only proposal voting is allowed with the voting key
                                  ...
                                  const CBudgetVote::VoteDirection& nVote, const Optional<std::string>& mnAliasFilter)
{
    ...
    mnKeyList mnKeys = fLegacyPN ? getPNKeys(mnAliasFilter, resultsObj, failed)
                                 : getDPNVotingKeys(pwallet, mnAliasFilter, fFinal, resultsObj, failed);
                                       ...
                                       const CBudgetVote::VoteDirection& nVote)
{
    ...
    mnKeyList mnKeys = fLegacyPN ? getPNKeysForActivePatriotnode(resultsObj)
                                 : getDPNKeysForActivePatriotnode(resultsObj);
