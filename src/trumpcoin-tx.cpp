...
static bool AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") || gArgs.IsArgSet("-help")) {
        ...
        std::string strUsage = PACKAGE_NAME " trumpcoin-tx utility version " + FormatFullVersion() + "\n\n" +
                               "Usage:  trumpcoin-tx [options] <hex-tx> [commands]  Update hex-encoded trumpcoin transaction\n" +
                               "or:     trumpcoin-tx [options] -create [commands]   Create hex-encoded trumpcoin transaction\n" +
                               "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded trumpcoin transaction
