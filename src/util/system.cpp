...
const char * const TrumpCoin_CONF_FILENAME = "trumpcoin.conf";
const char * const TrumpCoin_PID_FILENAME = "trumpcoin.pid";
const char * const TrumpCoin_PATRIOTNODE_CONF_FILENAME = "patriotnode.conf";


// TrumpCoin only features
// Patriotnode
std::atomic<bool> fPatriotNode{false};
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager& am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "trumpcoind -foo=bar
...
#else
    const char* pszModule = "trumpcoin";
#endif
    if (pex)
        return strprintf(
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\TrumpCoin
// Windows >= Vista: C:\Users\Username\AppData\Roaming\TrumpCoin
// Mac: ~/Library/Application Support/TrumpCoin
// Unix: ~/.trumpcoin
#ifdef WIN32
    // Windows
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\TrumpCoinParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\TrumpCoinParams
    // Mac: ~/Library/Application Support/TrumpCoinParams
    // Unix: ~/.trumpcoin-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "TrumpCoinParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectories(pathRet);
    return pathRet / "TrumpCoinParams";
#else
    // Unix
    return pathRet / ".trumpcoin-params";
#endif
#endif
}
...
void initZKSNARKS()
{
    ...
    if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
        ...
        sapling_spend = "/usr/share/trumpcoin/sapling-spend.params";
        sapling_output = "/usr/share/trumpcoin/sapling-output.params";
        if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
            fParamsFound = true;
        } else {
            // Linux fallback for local installs
            sapling_spend = "/usr/local/share/trumpcoin/sapling-spend.params";
            sapling_output = "/usr/local/share/trumpcoin/sapling-output.params";
        }
#endif
        if (fs::exists(sapling_spend) && fs::exists(sapling_output))
            fParamsFound = true;
...
fs::path GetPatriotnodeConfigFile()
{
    fs::path pathConfigFile(gArgs.GetArg("-mnconf", TrumpCoin_PATRIOTNODE_CONF_FILENAME));
...
fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", TrumpCoin_PID_FILENAME));
