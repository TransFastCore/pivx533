...
// static 0.1 TRUMP fee used for the special txes in these tests
...
static CMutableTransaction CreateProRegTx(Optional<COutPoint> optCollateralOut, SimpleUTXOMap& utxos, int port, const CScript& scriptPayout, const CKey& coinbaseKey, const CKey& ownerKey, const CBLSPublicKey& operatorPubKey, uint16_t operatorReward = 0)
{
    ...
    FundTransaction(tx, utxos, scriptPayout,
                    ...
                    (optCollateralOut ? 0 : Params().GetConsensus().nPNCollateralAmt));
...
static bool IsPNPayeeInBlock(const CBlock& block, const CScript& expected)
{
...
static void CheckPayments(std::map<uint256, int> mp, size_t mapSize, int minCount)
{
    ...
    for (const auto& it : mp) {
        BOOST_CHECK_MESSAGE(it.second >= minCount,
                strprintf("PN %s didn't receive expected num of payments (%d<%d)",it.first.ToString(), it.second, minCount)
        );
    }
}
...
BOOST_FIXTURE_TEST_CASE(dip3_protx, TestChain400Setup)
{
    ...
    const CSporkMessage& sporkMnPayment = CSporkMessage(SPORK_8_PATRIOTNODE_PAYMENT_ENFORCEMENT, nTime + 1, nTime);
    ...
    for (size_t i = 0; i < 6; i++) {
        ...
        BOOST_CHECK(deterministicPNManager->GetListAtChainTip().HasPN(txid));
    ...
    // enable SPORK_21
    const CSporkMessage& spork = CSporkMessage(SPORK_21_LEGACY_PNS_MAX_HEIGHT, nHeight, GetTime());
    sporkManager.AddOrUpdateSporkMessage(spork);
    BOOST_CHECK(deterministicPNManager->LegacyPNObsolete(nHeight + 1));

    // Mine 20 blocks, checking PN reward payments
    std::map<uint256, int> mapPayments;
    for (size_t i = 0; i < 20; i++) {
        SyncWithValidationInterfaceQueue();
        auto dmnExpectedPayee = deterministicPNManager->GetListAtChainTip().GetPNPayee();
        CBlock block = CreateAndProcessBlock({}, coinbaseKey);
        chainTip = chainActive.Tip();
        BOOST_ASSERT(!block.vtx.empty());
        BOOST_CHECK(IsPNPayeeInBlock(block, dmnExpectedPayee->pdmnState->scriptPayout));
        mapPayments[dmnExpectedPayee->proTxHash]++;
        BOOST_CHECK_EQUAL(chainTip->nHeight, ++nHeight);
    }
    // 20 blocks, 6 patriotnodes. Must have been paid at least 3 times each.
    ...
    // register multiple PNs per block
    for (size_t i = 0; i < 3; i++) {
        ...
        for (size_t j = 0; j < 3; j++) {
            BOOST_CHECK(mnList.HasPN(txns[j].GetHash()));
        }

        nHeight++;
    }

    // Mine 30 blocks, checking PN reward payments
    mapPayments.clear();
    ...
    // 30 blocks, 15 patriotnodes. Must have been paid exactly 2 times each.
    CheckPayments(mapPayments, 15, 2);

    // Check that the prev DPN winner is different that the tip one
    std::vector<CTxOut> vecMnOutsPrev;
    BOOST_CHECK(patriotnodePayments.GetPatriotnodeTxOuts(chainTip->pprev, vecMnOutsPrev));
    std::vector<CTxOut> vecMnOutsNow;
    BOOST_CHECK(patriotnodePayments.GetPatriotnodeTxOuts(chainTip, vecMnOutsNow));
    BOOST_CHECK(vecMnOutsPrev != vecMnOutsNow);

    // Craft an invalid block paying to the previous block DPN again
    ...
    // ProUpServ: change patriotnode IP
    {
        ...
        SyncWithValidationInterfaceQueue();
        auto dmn = deterministicPNManager->GetListAtChainTip().GetPN(proTx);
    ...
    // ProUpServ: Try to change the IP of a patriotnode to the one of another registered patriotnode
    {
        ...
        int new_port = deterministicPNManager->GetListAtChainTip().GetPN(dmnHashes[randomIdx2])->pdmnState->addr.GetPort();
    ...
    // ProUpServ: Try to change the IP of a patriotnode that doesn't exist
    {
        const CBLSSecretKey& operatorKey = GetRandomBLSKey();
    ...
    // ProUpServ: Change patriotnode operator payout. (new patriotnode created here)
    {
        ...
        SyncWithValidationInterfaceQueue();
        auto mnList = deterministicPNManager->GetListAtChainTip();
        BOOST_CHECK(mnList.HasPN(txid));
        auto dmn = mnList.GetPN(txid);
        ...
        BOOST_CHECK_EQUAL(chainTip->nHeight, ++nHeight);
        SyncWithValidationInterfaceQueue();
        dmn = deterministicPNManager->GetListAtChainTip().GetPN(txid);
        BOOST_ASSERT(dmn != nullptr);
        BOOST_CHECK(dmn->pdmnState->scriptOperatorPayout == operatorPayee);
    }

    // ProUpServ: Try to change patriotnode operator payout when the operator reward is zero
    {
        const CScript& operatorPayee = GenerateRandomAddress();
    ...
    // Block including
    // - (1) ProRegTx registering a patriotnode
    // - (2) ProUpServTx changing the IP of another patriotnode, to the one used by (1)
    {
        auto tx1 = CreateProRegTx(nullopt, utxos, port++, GenerateRandomAddress(), coinbaseKey, GetRandomKey(), GetRandomBLSKey().GetPublicKey());
        const uint256& proTx = dmnHashes[InsecureRandRange(dmnHashes.size())];    // pick one at random
    ...
    // ProUpReg: change voting key, operator key and payout address
    {
        ...
        // check that changing the operator key puts the PN in PoSe banned state
        ...
        BOOST_CHECK_EQUAL(chainTip->nHeight, ++nHeight);
        SyncWithValidationInterfaceQueue();
        dmn = deterministicPNManager->GetListAtChainTip().GetPN(proTx);
        ...
        // Mine 32 blocks, checking PN reward payments
        mapPayments.clear();
        ...
        // 16 patriotnodes: 2 rewards each
        CheckPayments(mapPayments, 16, 2);
    }

    // ProUpReg: Try to change the voting key of a patriotnode that doesn't exist
    {
    ...
    // ProUpReg: Try to change the operator key of a patriotnode to the one of another registered patriotnode
    {
        int randomIdx = InsecureRandRange(dmnHashes.size());
        int randomIdx2 = 0;
        do { randomIdx2 = InsecureRandRange(dmnHashes.size()); } while (randomIdx2 == randomIdx);
    ...
    // Block including
    // - (1) ProRegTx registering a patriotnode
    // - (2) ProUpRegTx changing the operator key of another patriotnode, to the one used by (1)
    {
        const CBLSSecretKey& new_operatorKey = GetRandomBLSKey();
    ...
    // ProUpRev: revoke patriotnode service
    {
        const uint256& proTx = dmnHashes[InsecureRandRange(dmnHashes.size())];            // pick one at random
