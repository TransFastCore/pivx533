// Copyright (c) 2018-2020 The TrumpCoin developers
// Distributed under the MIT/X11 software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include "test_trumpcoin.h"
...
void enableMnSyncAndSuperblocksPayment()
{
    ...
    spork = CSporkMessage(SPORK_9_PATRIOTNODE_BUDGET_ENFORCEMENT, nTime + 1, nTime);
    sporkManager.AddOrUpdateSporkMessage(spork);
    BOOST_CHECK(sporkManager.IsSporkActive(SPORK_9_PATRIOTNODE_BUDGET_ENFORCEMENT));
}
...
{
    const CAmount mnAmt = GetPatriotnodePayment();
    ...
    // enable SPORK_8
    int64_t nTime = GetTime() - 10;
    const CSporkMessage& spork = CSporkMessage(SPORK_8_PATRIOTNODE_PAYMENT_ENFORCEMENT, nTime + 1, nTime);
    sporkManager.AddOrUpdateSporkMessage(spork);
