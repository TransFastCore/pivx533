                                  ...
                                  CPatriotnodeBroadcast& mnbRet)
{
    ...
    if (!mnp.Sign(keyPatriotnodeNew, pubKeyPatriotnodeNew.GetID())) {
...
bool CPatriotnodeBroadcast::CheckAndUpdate(int& nDos)
{
    ...
    if (nMessVersion != MessageVersion::MESS_VER_HASH) {
        LogPrint(BCLog::PATRIOTNODE, "mnb - rejecting old message version for mn %s\n", vin.prevout.hash.ToString());
        nDos = 30;
        return false;
    }
    ...
    if (pubkeyScript.size() != 25) {
        LogPrint(BCLog::PATRIOTNODE,"mnb - pubkey the wrong size\n");
        nDos = 100;
        return false;
    }
    ...
    if (pubkeyScript2.size() != 25) {
        LogPrint(BCLog::PATRIOTNODE,"mnb - pubkey2 the wrong size\n");
        nDos = 100;
        return false;
    }
    ...
    if (!CheckSignature()) {
        ...
        nDos = patriotnodeSync.IsSynced() ? 100 : 5;
        return error("%s : Got bad Patriotnode address signature", __func__);
    }
...
bool CPatriotnodePing::CheckAndUpdate(int& nDos, bool fRequireAvailable, bool fCheckSigTimeOnly)
{
    if (sigTime > GetMaxTimeWindow()) {
    ...
    if (sigTime <= GetAdjustedTime() - 60 * 60) {
        LogPrint(BCLog::PNPING,"%s: Signature rejected, too far into the past %s - %d %d \n", __func__, vin.prevout.hash.ToString(), sigTime, GetAdjustedTime());
        nDos = 30;
        return false;
    }
    ...
    // reject old signature version
    if (nMessVersion != MessageVersion::MESS_VER_HASH) {
        LogPrint(BCLog::PNPING, "mnp - rejecting old message version for mn %s\n", vin.prevout.hash.ToString());
        nDos = 30;
        return false;
    }
