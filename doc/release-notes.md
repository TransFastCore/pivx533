...
The `autocombinerewards` RPC command was soft-deprecated in v5.3.0 and replaced with explicit setter/getter commands `setautocombinethreshold`/`getautocombinethreshold`. TrumpCoin Core, by default, will no longer accept the `autocombinerewards` command, returning a deprecation error, unless the `trumpcoind`/`trumpcoin-qt` is started with the `-deprecatedrpc=autocombinerewards` option.
